...
\begin{align}
  \sigma &\sim \text{Student's T}\left(\nu = 3, 0, s = 1.5\right)
  (\#eq:scale-cauchy)
\end{align}
...
It is also important to note that these estimates control for incumbency, so the utility peak at party-median CF score values _does not_ merely reflect the fact that incumbent candidates represent the ideological "core" of the party.
...
The utility function for districts with average district-party conservatism is almost identical to the utility functions for districts with above-average and below-average district-party conservatism.
More conservative candidates are not noticeable more valuable to conservative primary electorates, nor are progressive candidates more valuable to progressive electorates.

These results are partially consistent with the strategic positioning dilemma.
...
- distance contains both CF and theta
- results show the function responds mostly to CF, barely any effect of theta
- appearance of an optimum; candidates are more valuable in a particular region of ideology space, less valuable in the fringes
- NO effect of 
...
### CF scores and causal identification

Centrist CF score reflects a consensus view that candidate is viable, so there remains some confounding.

Feedback over time. If candidates observed that moderates were winning primaries, or extremists were winning primaries, they would shift their ideologies so that these ideal points became the center of the party.
