...
The strategic positioning dilemma view of U.S. primaries has questionable theoretical underpinning.
...
styles, such as "delegate" versus "trustee" models of representation, or "descriptive" versus "substantive" representation [@burke:2012:trusteeship
[@pitkin:1967:representation; @phillips:1995:politics-presence; @canon:1999:racial-redistricting]. 
...
These voters are more likely to turn out in general elections than moderate voters are, so it it is more efficient for candidates to cater to these constituents. 
...
outward pressures from ideological primary voters.
...
Qualitative evidence suggests that candidates take careful positions on issues of local concern even if these delicate tactics don't influence high-level relationships between local preferences and legislator behavior [@fenno:1978:home-style], but systematic tests of localized, particularistic position-taking are mixed.
- faithfulness to local opinion on specific issues [@canes-wrone-et-al:2011:issue-accountability]
- not much convergence even where you "most expect it" @fowler-hall:2016:convergence].
...
<!------- TO DO ---------
- transition to sophistication argument
------------------------->
...
which denies voters' the informational shortcut of a candidate's party affiliation.
<!------- TO DO ---------
- cite party label?
------------------------->
<!------- TO DO ---------
...
This is because primary voters are commonly believed to hold more ideologically consistent policy views than other constituents, so candidate polarization will respond to the polarization among the voting public [@hill:2015:nominating-institution].
<!------- TO DO ---------
- who does hill cite? 
- Norrander 199x?
- wo else?
------------------------->
However, these is essentially no recent evidence to support the argument that primary rules contribute to the polarization of congressional candidates or candidate divergence more broadly.
<!------- TO DO ---------
- mcghee, hill, sides et al on variation by rules
- any hirano stuff?
- check hirano book or boatright book
------------------------->
although other research disputes the premise that primary voters are distinct demographically or ideologically from other partisans
[@sides-et-al:2018:primary-representativeness]



Some states also have conventions that screen certain candidates out of the primary process altogether, so candidates on the primary ballot already have some degree of approval from an insider class of state party actors.
<!------- TO DO ---------
- cite conventions?
------------------------->
...
------------------------->
Studies routinely acknowledge this distinction in theory, but they often abandon the distinction between the two groups applied studies, instead operationalizing the preferences of all three constituencies---the general constituency and two partisan primary constituencies---using the same single and inappropriate measure: the district-level presidential vote.
The presidential vote share is not suitable for the study of primary representation for the simple reason that votes are not equivalent to policy preferences or political ideology.
(I refer to "ideology" in its meaning as a summary measure of an individual's issue preferences---i.e. an "ideal point"---rather than its meaning as an abstract political value set from which policy positions can be derived.)
<!------- TO DO ---------
- section about the use of the term "ideology?"
------------------------->
Republican voters in a district may be ideological moderates or ideological conservatives, but the fact that they all vote Republican does not inform us on their relative ideological tastes. 
Similarly, a district's vote outcome captures how its constituents vote on average, but because partisans tend to vote foremost for their preferred party, this average may be more strongly affected by number of voters in each party rather than their ideological preferences.
<!------- TO DO ---------
- simulation
------------------------->
...
Measurement method uses weak data plus pooling

- variation is smoothed by a hierarchical model
- exogenous variation for design-based causal inference would have to come predominantly through the raw survey data, which is challenging to recover without massive enough data to rely less on the smoothing model
- as a result, we rely on selection on observables methods, 

Selection on observables is the graveyard of credible causal inference.

- many regression-focused analyses are not careful about their strategies for including and excluding regressors
- many regressors are naively included in linear forms without much thought to their generative influence on the outcome or potential interactions with other confounders
- functional forms are generally under-examined

One major ethic guiding the methodological contributions in this project is to take the modeling approaches more seriously from a causal inference point of view.

- This means stating assumptions clearly
- being explicit about the stipulated causal structure
- where are certain controls located in the causal structure
- which estimands are implied by the causal structure
- which estimands assumptions identify these estimands
- which modeling approaches are flexible enough to absorb confounding without too much model dependence
...
Recent years have seen rapid progress in the development and understanding of Hamiltonian Monte Carlo algorithms, which are faster, more statistically reliable, and easier to diagnose [@duane-et-al:1987:hybrid-mc; @neal:2012:mcmc; @betancourt:2017:conceptual-hamiltonian], but they also require renewed attention to the way researchers specify and implement Bayesian models [@betancourt:2015:hamiltonian; @carpenter-at-al:2016:stan; @burkner:2017:brms].
