...
Policy ideology affects voters' issue beliefs, and while issue beliefs can be measured using a survey, policy ideology itself cannot be.
...
This project measures primary candidates' campaign positioning using CFscores from Bonica's -@bonica:2019:dime _Database on Ideology, Money in Politics, and Elections_ (DIME) database.
CFscores use campaign contributions to measure the political ideologies of contributors and recipients of campaign contributions, including candidates for office, party organizations, PACs, and individual donors. 
The estimation method assumes that a donor contributes to potential recipients in a way that maximizes utility of their total contributions, which is affected by the ideological similarity between donors and potential recipients [@bonica:2013:ideology-interests; @bonica:2014:mapping-ideology].
...
As I discuss in future chapters, CFscore are not without controversy as indicators of elite ideology, especially when comparing members of the same party [@tausanovitch-warshaw:2017:polarized-congress; @hill-huber:2017:donorate], but research shows that donors differentiate moderate and ideological candidates within the same party [@barber-et-al:2016:ideological-donors], the ideology component of CFscores outperforms a party-only model of giving [@bonica:2014:mapping-ideology], and CFscores predict future votes by members of Congress to a similar degree of accuracy as roll-call based scores do [@bonica:2019:cf-validity].
...
Classic formal models highlight a strategic logic for candidates to position themselves by "converging" to the location of the median voter: if constituents vote primarily with policy-based or ideological considerations, then candidates maximize the probability of electoral victory by positioning themselves as closely to the median constituent as possible [@downs:1957:economic-theory; @black:1948:group-decisions].^[
  ...
  The incentives for moderation are more theoretically important than the whether we observe perfect candidate convergence at the median voter. 
]
...
Partisan constituents are also more likely to engage in pro-party activism, such as staffing campaigns and attending party conventions [@aldrich:1983:downsian-parties; @mcclosky-et-al:1960:conventions; @layman-et-al:2010:activists-conflict-extension].
...
Primaries may also promote divergent candidate stances merely because candidates must rely on committed partisans to staff their campaigns [@layman-et-al:2010:activists-conflict-extension; @aldrich:2011:why-parties]. 
Through these mechanisms, candidates can live or die by the nomination process long before primary voters ever enter the picture.
...
------------------------->
Party labels on the ballot are valuable heuristics for voters to differentiate candidates; voters can be broadly confident about the issue disagreements that distinguish Republican candidates from Democratic candidates  [@hill:2015:nominating-institution].
...
------------------------->
In these low-information environments, voters may decide to cast their ballot for various non-policy reasons [@porter-treul:2020:primary-experience; @thomsen:2020:ideology-gender].
They may vote for the familiar candidate instead of the ideologically proximate one, in which case asymmetric campaign expenditures or news coverage may advantage one candidate over the other [e.g. @bonica:2020:lawyers-in-congress].
<!------- TO DO ---------
- name recognition
------------------------->
...
This is because there isn't much consensus in the opinion research that partisans who participate in primaries are that different from partisans who don't participate in primaries, either demographically or ideologically [@geer:1988:primary-electorates; @norrander:1989:primary-voters; @sides-et-al:2018:primary-representativeness].
And even scholars who do find that primary voters hold more ideological views nonetheless find no evidence that closed primaries nominate candidates that are more ideologically off-center [@hill:2015:nominating-institution].
...
Studies routinely acknowledge this distinction in theory, but they often abandon the distinction between the two groups applied studies, instead operationalizing the preferences of all three constituencies---the general constituency and two partisan primary constituencies---using the same measure: the district-level presidential vote.
...
Using the terminology by @tomz-van-houweling:2008:candidate-positioning, studying vote shares rarely presents a "critical test" of theories of voting, since the same observable vote outcome can arise from many underlying voter preference configurations.
...
Republican-identifying constituents hold policy preferences that are more conservative than Democratic constituents on average: the median Republican and Democrat are respectively located at `r -1 * median_same` and `r median_same`.^[
  I use Normal distributions, so the median of each distribution is also the mean, but I refer to the median of each distribution since medians are more directly relevant to models of spatial voting.
]
Each distribution has a scale parameter of `r sd_same`, so there is some overlap where some Democratic constituents are more conservative than some Republican constituents, despite their party affiliation.
...
The final bottom-right panel in the figure shows how we can obtain a different district vote without changing the underlying ideological distribution in either party whatsoever, instead changing only the relative population size of each partisan base.
...
In every panel, we observe an increase in the Democratic vote compared to our baseline scenario, but in almost no cases does this mean that the median voter in either party must have changed.
...
The median Democratic ideal point is affected only by location-shifting the Democratic distribution to the left, not by lowering the Democratic ideal point variance or by increasing the size of the Democratic constituency. 
...
```{r plot-compare-measures, include = TRUE, fig.width = 10, fig.height = 6, out.width = "100%", fig.scap = "The relationship between average ieological self-placement and district vote share in congressional districts", fig.cap = "Average ideological self placement (vertical axis) and Republican vote share (horizontal axis) in all 435 congressional districts. Mean self-placement is calculated by numerically coding CCES ideological self-placement responses before averaging. The first panel plots average self-placement among all CCES respondents in each congressional districts. The middle panel breaks respondents in each congressional district into Republican and Democratic subgroups before averaging. The final panel plots loess fits for the same relationship measured over all CCES years."}
```
...
their party's base of support against other voters in their districts, we must be able to measure the preferences of a politician's local partisan constituency.

and its omission of primary candidate ideology have prevented scholars from testing basic theoretical propositions in the study of primary politics.
...
Because we have not studied primary constituency preferences, we cannot characterize their impact in electoral representation.
...
An important missing piece from these studies is district-party ideology.
We should not expect a relatively moderate partisan constituency to nominate an extremist candidate solely because the primary is closed to non-party members.
Past studies have had to paper over the ideological variation across districts or use unsuitable proxy measures that do not measure district-party ideology.
...
The measurement model smooths estimates using hierarchical regression, using partial pooling to improve the estimate for one unit "borrowing information" from other units.
...
Given these data limitations, this project turns to causal identification through a "selection on observables" assumption.
...
One guiding ethic for the methodological contributions in this project is to take observational causal modeling more seriously than the existing research on primary representation by setting up empirical analysis that aspire to do the following: 

- are explicit about the potential outcomes model that links treatments, outcomes, and confounders.
- are explicit about the causal estimand implied by a causal structure.
- are explicit about the assumptions required to identify estimands and how modeling approaches relate to identification assumptions.
...
I hope to satisfy these aims by invoking more explicit models of potential outcomes [@rubin:2005:potential-outcomes] and using "structural causal models" (SCMs) to guide model specification choices [e.g. @pearl:1995:causal-diagrams].
...
This project's approach to causal has two stand-out contributions to the study of primary representation that would be impossible but for this approach.
...
I lay out the causal structure in causal graphs, discuss identification assumptions required to estimate the causal estimand of interest, and implement a sequential-$g$ modeling approach to estimate it [@acharya-blackwell-sen:2016:direct-effects].
Chapter \@ref(ch:voting) explores flexible modeling with machine learning as a way to reduce dependence on fragile model assumptions. 
The analysis uses a two-stage modeling approach for treatment assignment and treatment effects that de-biases causal estimates from regularized machine learning methods, which is discussed in other areas of machine learning econometrics but almost nowhere in political science [@hahn-et-al:2018:regularization-confounding; @wager-athey:2018:honest-causal-forests; @ratkovic:2019:rehabilitating-regression].
...
ALthough Bayesian approaches to causal inference is not new [@rubin:1978:bayesian], it appears almost nowhere in political science. 
...
I give pragmatic guidance for thinking about priors and specifying Bayesian causal models, and I demonstrate the added value of a Bayesian approach by extending the analyses of published political science work using regression discontinuity and the meta-analysis of randomized experiments.
