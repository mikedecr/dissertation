# How District-Party Ideology Affects Primary Candidate Positioning {#ch:positioning}
...
For this to be true, we should expect that the policy ideology within a partisan constituency affects the ideological positioning of candidates who run for that party's nomination. 
...
Does a candidate for Congress stake out a moderate stance because of a close balance between the number of Republicans and Democrats in their district, or because the voters in their own party are relatively moderate compared to partisans in other districts?
Although past research has been interested in the threat of primary challenges as a cause of ideological divergence between partisan legislators [for example @boatright:2013:getting-primaried; @hill:2015:nominating-institution; @hirano-et-al:2010:primary-polarization; @mcghee-et-al:2014:nomination-systems], many of these studies lacked the capability to observe the preferences within local partisan groups as a concept distinct from aggregate partisanship or aggregate voting in the entire district.
...
We cannot directly compare the "explanatory power" of district-party ideology to the explanatory power of district-level partisanship by observing that one is more strongly correlated with candidate ideal points than the other, nor can we simply control for aggregate voting to recover the "partial effect" of district-party ideology.
This is because policy ideology and aggregate voting should themselves be causally related: if a district contains a voter base with more conservative policy preferences, those voters probably also vote more heavily Republican, and candidates position themselves to anticipate those preferences.
...
This could be a within-party Downsian incentive: the median primary voter is an ideological partisan with off-median policy preferences, so candidates take more extreme positions to appeal to the median partisan
...
They further find no evidence that incumbent members of Congress have more extreme NOMINATE scores under greater threats of primary competition or higher primary turnout.
...
The IRT measures of district-party ideology that I create in Chapter \@ref(ch:model) are superior measures of policy preferences to ideological self-placement scores and additive issue scores, which respectively tap "symbolic" ideology or suppress all item-level opinion variation [@ellis-stimson:2012:symbolic-ideology; @treier-hillygus:2009:ideology].^[
  @erikson-et-al:1993:statehouse-democracy also use surveys to summarize partisan opinion within states without modeling item effects.
]
...
This process could weaken the relationship between district-party ideology and candidate positioning because moderate candidates select themselves out of the campaign altogether.
...
This non-relationship may indicate that the positioning of inexperienced candidates may be less related to district-party ideology, perhaps because inexperienced candidates are not adept at perceiving local ideology [@ensley-et-al:2009:complexity], they do not have the resources to conduct or acquire surveys of their constituencies, or they make non-ideological appeals in their campaigns. 
Incumbents, in turn, may be more responsive to local ideology as a result of their political skills and survivorship bias—ill-fitting candidates do not become incumbents in the first place—although incumbents systematically misperceive public opinion as well [@broockman-skovron:2018:bias].
...
There are many more states whose primary rules fall between these two extremes, such as "semi-closed" systems that allow party-unaffiliated voters to choose the party in whose primary they wish to participate [@mcghee-et-al:2014:nomination-systems].
Political scientists and political observers speculate that these rules shape candidates' positioning incentives by changing the composition of the primary electorate.
...
More open systems, especially "blanket" systems where candidates from all parties run on the same primary ballot to advance to the general election, encourage primary candidates to take more moderate stances to attract an ideologically diverse primary coalition.
...
Estimating an IRT ideal point model on individual CCES respondents in each congressional district with validated primary turnout data, Hill finds that primary voters are more ideologically consistent than general election voters in the same party, but primary voters are no more extreme in closed primary states than in open primary states.
Even if primary electorates are not affected by state primary rules, candidates may still believe that primary rules matter and position themselves accordingly.
...
Looking at the mechanisms underlying this, however, @hill:2015:nominating-institution, who finds no effect on the ideological composition of primary voters after California's reforms, and survey experiments by @ahler-citrin-lenz:2016:CA-open-primaries broadly show that voters are unable to identify which candidates are more ideological and which are more moderate in a top-two primary context.

I leverage my new data on district-party ideology to test a variation on the primary rules hypothesis in Section \@ref(sec:g-modification) below.
Rather than study whether primary rules lead candidates to be more or less extreme, I study whether stricter primary rules increase candidates' responsiveness to district-party ideology by making the district-party "more important" to primary success.
Unlike the pattern of results from other studies, I find that candidates are more responsive to district-party ideology in states with stricter primary rules.
...
An increasingly prominent theoretical perspective in U.S. political research holds that the parties are not mirror images of one another but instead exhibit many "asymmetries" that explain the contours of recent political conflict.
...
Confidence intervals reflect standard errors that are clustered at the district-party level to capture correlated error among candidates who to represent the same constituency. 
...
The higher correlation among Democrats also disappears among incumbent candidates, suggesting that no party is obviously more responsive to primary constituencies than the other, contrary to Clinton's -@clinton:2006:constituent-representation finding that Republican incumbents were uniquely sensitive to ideological variation within their partisan bases.
...
This would be consistent with the key finding from @boatright:2013:getting-primaried that primary challenges focused purely on ideological fit to the district are relatively rare.
...
Almost all relationships are statistically significant at a $1$% level except among open seat candidates, whose sample sizes are also smaller.
...
There is no definitive trend toward increasing polarization or increasing responsiveness to partisan ideology in more recent years, although a longer time series spanning multiple decades might reveal slower-moving trends.
...
Data on primary systems come from @boatright-et-al:2020:primary-timing-data for 2012 and 2014, and I coded 2016 by consulting the National Conference of State Legislators, Ballotpedia, and OpenPrimaries.org.^[
...
Figure \@ref(fig:plot-rules-means), which plots the average CF score for Republicans and Democrats under each primary system, provides contradictory evidence against this hypothesis.^[
...
The relationships do not support the primary rules hypothesis either.
While Republican candidates are indeed least sensitive to local partisan ideology in states with the most open rules—indeed the point estimate of the relationship is negative—they are more sensitive in _semi_-closed rather than closed states.
...
Because these partisan individuals are also members of the district constituency, intervening on the party location also affects the district location, justifying the causal path $\bar{\theta}_{g} \rightarrow V_{d}$.
...
This is similar to the motivation of @clinton:2006:constituent-representation, who compared incumbent responsiveness to district median preferences versus party median preferences by regressing NOMINATE scores on both preference measures.
...
This problem is diagrammed in the right half of Figure \@ref(fig:combo-dag), which shows how conditioning on the presidential vote, a measure of $V_{d}$, opens a new pathway $\bar{\theta}_{g} \rightarrow V_{d} \rightarrow U \rightarrow \text{CF}_{i}$, biasing the estimated causal effect of $\bar{\theta}_{g}$.
At the same time, doing nothing to adjust for aggregate district preferences may identify the _total_ effect of the district-party constituency under certain assumptions, but it would not capture how much of the effect is mediated through the presidential vote, so there would be no way to isolate the unique impact of the partisan constituency.
...
\begin{align}
  \text{CDE}_{i}\left(\theta, \theta', v \right) &= 
...
Intermediate confounders are allowed to be affected by both the treatment and pre-treatment confounders.
...
```{r plot-g-dag, include = TRUE, fig.width = 9, fig.height = 10, out.width = "100%", fig.scap = "Sequential $g$-estimation using causal diagrams.", fig.cap = "Causal graphs describing the modeling problem and sequential $g$-estimation. Stage 1 identifies the effect of the past district voting ($V_{d}$) on candidate positioning ($\\text{CF}_{i}$). Stage 2 subtracts the district vote effect from candidate positions and identifies the effect of district-party ideology $\\bar{\\theta}_{g}$ on the demediated CF score $b(\\text{CF})_{i}$, which is equivalent to the controlled direct effect on the raw CF score. The final graph shows where unobserved confounders violate the causal identification assumptions in stage 1 ($U1$) and in stage 2 ($U2$)."}
```
...
Furthermore, there remains a causal path from the intermediate confounders $Z_{d}$ to the CF score, but the stage-two model does not condition on these confounders because these pathways are contained in the ACDE of district-party ideology on CF scores.
...
This quantity can be valuable because the difference between the total effect and the controlled direct effect shows how much of the total effect is carried by a mediating mechanism.
...
With few exceptions, it is not typical for research on primary politics to incorporate explicit causal inference methodologies.
The studies that do incorporate causal methods tend to use difference-in-differences, regression discontinuity, or survey experiments [@hall:2015:extremists; @fowler-hall:2016:convergence; @hall-thompson:2018:who-punishes; @doherty-et-al:2019:party-chairs] rather than structural modeling.
Structural modeling is important, however, to ensure that selection-on-observables designs identify the correct causal quantity and perform the appropriate statistical adjustments to estimate it. 
...
The average controlled direct effect of district-party ideology on primary candidate CF scores is identified nonparametrically under SUTVA, sequential ignorability, and positivity.
...
One goal for future work in this literature would be to combine the sequential $g$ structural model with machine learning methods to relax these functional form assumptions, a nascent area of work that exists largely outside of political science [@hill:2011:bart; @athey-at-al:2016:residual-balancing; @samii-et-al:2016:retrospective-causal-inference-ML; @chernozhukov-et-al:2017:double-ML; @wager-athey:2018:honest-causal-forests; @ratkovic:2019:rehabilitating-regression; @hahn-et-al:2020:bayesian-causal-forests]
...
The data contain observations of candidates $i$ in district-party groups $g$ in districts $g$.
...
Because the estimation proceeds in two stages of regression modeling, I sometimes subscript parameters with $1$ or $2$ to indicate which equation they belong to.

The first stage is a mediator–outcome model that estimates how the Republican two-party presidential vote in the past election $V_{d}$ affects the CF score of candidate $i$ within group $g$, controlling for pre-treatment confounders $\mathbf{x}_{d}$ and intermediate confounders $\mathbf{z}_{d}$.
...
------------------------->
The group ideal point $\bar{\theta}_{g}$ is included in the regression as a control, so its coefficient $\eta$ is estimated as a nuisance parameter, as are the coefficients $\beta_{1}$ for pre-treatment confounders, the coefficients $\gamma$ for intermediate confounders, and the constant $\zeta_{1}$.
Because the mediator is measured at the district level, I include a district error term $\alpha_{d}$ in addition to the observation-level error term $\epsilon_{i}$.
...
We calculate the demediated outcome $b(\text{CF})_{i}$ by subtracting each district's demediation function from the candidate outcome value.
\begin{align}
...
\begin{split}
  b\left(\mathrm{CF}\right)_{i} &= 
    \zeta_{2} +
    \tau \bar{\theta}_{g} + 
    \mathbf{x}^{\intercal}_{g[i]}\beta_{2} + 
    \omega_{d[i]} + u_{i} \\
  \omega_{d} &\sim \text{Normal}\left(0, \sigma_{\omega}\right) \\
  u_{i} &\sim \text{Normal}\left(0, \sigma_{u}\right)    
\end{split}
(\#eq:trt-model)
\end{align}
where $\zeta_{2}$ is a constant, $\tau$ is the coefficient for district-party ideology, $\beta_{2}$ are coefficients for pre-treatment confounders, $\omega_{d}$ is a district error term, and $u_{i}$ is an observation-level error term.
...
More generally, the ACDE of setting district-party ideology from $\bar{\theta}'$ to $\bar{\theta}$ is as follows.^[
  ...
  The linear model with no treatment interactions produces a simple linear ACDE definition, but a more complex model would entail a different formula.
]
\begin{align}
...
Chapter \@ref(ch:causality) describes a number of special advantages for Bayesian causal modeling, stemming from the fact that Bayesian inference allows the researcher to conduct posterior inference about causal effects using probabilities: the treatment effect is _probably_ greater than $x$, with "probably" defined in relation to an empirical cumulative probability distribution function of the posterior distribution.
...
This is valuable because uncertainty in stages 1 and 2 are directly related to one another by way of the demediation function: stage 1 uncertainty affects the demediation function, and uncertainty in the demediation function affects stage 2. 
While non-Bayesian estimation requires either ad-hoc variance corrections for the multistage model or a bootstrapping approach [@acharya-blackwell-sen:2016:direct-effects], the posterior distribution from the Bayesian model captures all variances and covariances among model parameters inherently.
...
I choose 90% intervals because they are numerically more stable when estimated from sample draws.
...
Because the IRT model partially pools ideal points using a hierarchical Normal regression, the multivariate Normal prior is a reasonable approximation to the posterior distribution from the ideal point model.
...
Past research has explored the use of inverse-variance weighting to downweight observations with greater measurement uncertainty [e.g. @adolph-at-al:2003:wls-ei], which accomplishes a similar goal as the prior distribution but throws out all prior information about the covariance between ideal points.
...
This is similar in spirit to "multiple overimputation" [@blackwell-et-al:2017:measurement-error], which extends multiple imputation to a measurement error context setting by iteratively replacing mismeasured observations with draws from their posterior distribution.
...
Stated differently, if ideal points are related to candidate positioning, and ideal points are measured with error, then observing candidate positions can update our information about ideal points.
By specifying the prior directly and updating the parameters, the posterior distribution represents all sources of uncertainty without any need for additional imputation steps or model averaging [@gelman-hill:2006:multilevel p. 542].
...
Furthermore, most of the outcome data in each model fall mostly in the interval $[-2, 2]$, so we can place standard Normal priors on the covariates without being overly informative about the covariate effects and introducing regularization bias [@hahn-et-al:2018:regularization-confounding].
Constants are given wider priors to account for the fact that indicator variables are not exactly centered at their means.
\begin{align}
\begin{split}
  \zeta_1, \zeta_2 &\sim \text{Normal}\left(0, 5\right) \\
  \eta, \mu, \beta_{1}, \gamma &\sim \text{Normal}\left(0, 1 \right) \\
  \tau, \beta_{2} &\sim \text{Normal}\left(0, 1 \right)
\end{split}
(\#eq:g-coef-priors)
\end{align}
...
\begin{split}
  ...
  \omega_{d}  &\sim \text{Normal}\left(0, \sigma_{\omega} \right) \\    \sigma_{\omega} &\sim \text{Half-Cauchy}\left(0, 1 \right)
\end{split}
(\#eq:g-ranef-priors)
\end{align}
...
On the statistical front, multilevel models may bias coefficient estimates if the aggregate errors are not exchangeable.
...
Both of these model specifications require an exchangeable errors assumption at the group level.
...
Even though the multilevel model has similar assumptions as a regression on group averages, it has certain benefits that are convenient for these data.
Because the number of candidates in a district is not fixed across all districts, we would expect heteroskedasticity in a regression-on-averages model because some districts would have higher variances due to fewer candidates.
...
This has a similar intuition as a weighted least squares regression on group-averaged data, where groups with more observations are more informative about global parameters and receive greater weight.
...
<!-- Group size may also affect treatment assignment.
...
These covariates were organized and matched to primary candidacies by the Primary Timing Project by @boatright-et-al:2020:primary-timing-data.
District demographic variables (sourced originally from the American Community Survey) include district median income, population density, and land area, as well the percent of a district population that is White, Latino, college educated, below the federal poverty line, unemployed, employed in the service industry, employed in blue-collar jobs, aged 18–24, and aged 65+.
The @boatright-et-al:2020:primary-timing-data data also provide Mayhew's -@mayhew:1986:placing-parties five-level "traditional party organization" and binary "persistent factionalism" classifications for state level [@mayhew:1986:placing-parties].
...
This makes it easier to specify priors for coefficients, since standardized coefficients are unlikely to exceed $1$ except when predictors are highly correlated.
...
CF scores are measured on their original scale, which spans roughly as wide as $-5$ to $5$ across both parties, but the vast majority of Democrats occupy values in $[-2, 0]$ and Republicans in $[0, 2]$.
<!------- TO DO ---------
...
kable(
  vb_kable,
  caption.short = "Sample sizes in all sequential $g$ models.",
...
Variational estimators are asymptotically consistent and can be used to estimate any model written in Stan, but because they require approximations, samples from the approximate distribution tend to underestimate the variance in the true posterior distribution [@wang-titterington:2012:VB-asymptotics].
The benefit, however, is that models that would take several hours to estimate using MCMC can be estimated using VB in roughly one minute, which is extremely valuable for building and estimating the 14 models included in this chapter.
...
The controlled direct effect of district-party ideology is positive, with a posterior mean indicating that one-unit increase in district-party ideology causes a 
...
Progressivism among Democrats is greater in districts with greater density, less land area, and greater numbers of service sector employees, which are likely to be more urban districts, and in districts with more college graduates.
...
Some interesting or counter-intuitive findings are that Democrats are more progressive in districts with more more blue-collar workers, and they are more conservative in districts with higher poverty.
TPO levels 1 to 5 indicate increasing levels of party organization in a state, indexing a state-party's autonomy from the national party, organizational duration, internal hierarchical structure, coordination to nominate particular candidates across a wide variety of offices, and provision of material incentives to organizers [@mayhew:1986:placing-parties].
Coefficients on the TPO indicators do not suggest that states with stronger historical party organizations always result in more progressive Democrats; the large point estimate for TPO-3 states (mid-level party organization) is driven largely by the small number of candidates from TPO-3 states.

```{r, eval = FALSE}
full_data %>% count(party, tpo)
```
...
This reflects a perspective that incumbency selects for candidates that take median positions that ensure their reelection, while challengers must take more innovative and extreme stances to garner attention [@groseclose:2001:candidate-location].
@burden:2004:candidate-positioning finds instead that incumbents take more partisan positions, perhaps because incumbency provides other advantages that give incumbents more security to take positions that do not match district preferences as closely.
...
The findings in this study contain a mixture of evidence for and against each of these views.
...
Open seat candidates behave more similarly to incumbents than to challengers, which contradicts the finding by @ansolabehere-et-al:2001:candidate-positioning that open seat candidates may be most "out of step" with district preferences, and may be more consistent with the @burden:2004:candidate-positioning argument that non-incumbents position themselves most aggressively to match voter preferences because they have fewer built-in valence advantages.
...
Results among Republican open-seat candidates are difficult to interpret, showing a negative effect of Republican district voting and no clear effect of district-party conservatism.
Overall, these results suggest that nearly all candidates of all incumbency status are responsive on average to district-party ideology, which supports an SPD view of primary competition.
...
My results in Figure \@ref(fig:plot-coef-rules) suggest that the inclusion of district-party preferences is consequential for understanding primary openness, measuring the _responsiveness_ of candidates to district-party ideology across primary rules instead of the _extremism_ of candidates on average.
...
- you might say "this could be bullshit" 
  but that is not exactly constructive
...
First, I employ novel measures of district-party ideology, a concept that is essential to the SPD theory but rarely operationalized in existing studies to date.
Second, I advance the causal credibility of these studies by proposing a specific causal estimand and implementing a statistical approach that estimates it under explicitly stated assumptions.
And third, I demonstrate how to bring this method into a Bayesian causal framework, incorporating measurement error in the key independent variable as a prior distribution and obtaining a posterior distribution that quantifies uncertainty in all model parameters.
I find that primary candidates do position themselves to fit district-party policy preferences, even when controlling for aggregate district voting as an intermediary causal mechanism.
Furthermore, I find that candidate responsiveness is generally greater in states with stricter primary participation rules, a hypothesis that is common among political punditry but otherwise unsubstantiated by quantitative research on primary elections.
...
Whether this theoretical perspective jeopardizes the causal interpretation of the preceding analysis depends on where policy-demanding groups are located in a causal diagram.
...
Because the identification assumptions, demediation function, and ACDE are nonparametrically defined in terms of expectations about data, the method could be implemented using more flexible estimators of conditional expectations such as matching estimators or machine learning models.
