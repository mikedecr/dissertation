...
Total model uncertainty includes the measurement error in district-party ideal points, whose posterior distributions from the IRT model in Chapter \@ref(ch:model) serve as priors in the causal analysis.
...
party_fits <- full_data %>%
  ...
  mutate(
    ...
    lmhat = map2(
      ...
      .y = lmglance,
      .f = ~ {
        tstat <- qt(p = 0.975, df = .y$df.residual) 
        augment(.x) %>%
        mutate(
          lower = .fitted - tstat*.se.fit,
          upper = .fitted + tstat*.se.fit
        )
      }
    )
  ) %>%
...
\begin{align}
  ...
  (\#eq:post-acde)
\end{align}
which is an average of all possible $\tau$ values, each value weighted by the posterior probability of the associated $\boldsymbol{\pi}$ values.
...
Recall from the group IRT model in Chapter \@ref(ch:model) that district-party ideal points are not estimated precisely. 
Instead, ideal points are identified only up to a posterior distribution, with uncertainty that reflects prior ignorance and a finite sample of polling data.
<!------- TO DO ---------
...
Rather than jointly estimate the ideal point model and all subsequent causal analyses, which would be an overwhelming feat of model-building and estimation, I specify a multivariate Normal prior on the vector of ideal points.
\begin{align}
  \Theta &\sim 
    \mathrm{MultiNormal}\left( \hat{\Theta}, \hat{\Sigma}_{\Theta} \right) %_
  (\#eq:theta-prior)
\end{align}
The parameters in the prior are estimates from MCMC samples from the ideal points. 
The mean vector $\hat{\Theta}$ contains the estimated means for each ideal point, and the matrix $\hat{\Sigma}_{\Theta}$ contains variances for each ideal point's MCMC samples on the main diagonal and covariances between any two ideal points' MCMC samples on the off-diagonals.
Because the IRT model partially pools ideal points using a hierarchical Normal regression, it is a reasonable approximation to give the ideal points a Normal prior for subsequent causal analyses.

Specifying priors for ideal points is a method for measurement error 

<!------- TO DO ---------
- contrast to WLS methods
- contrast to propagated uncertainty?
- contrast to ABS
------------------------->
@acharya-blackwell-sen:2016:direct-effects derive an analytical variance estimator that deals with this, but unfortunately this estimator does not incorporate the additional uncertainty introduced by the fact that the key independent variable, district-party ideology, is an estimate from a separate measurement model.
...
\begin{align}
  a_0, \eta, \mu, \beta, \gamma &\sim \mathrm{Normal}\left(0, 10 \right)
  \alpha_{d} &\sim \mathrm{Normal}\left(0, \sigma_{\alpha} \right)
  \sigma_{\alpha} &\sim \mathrm{Normal}\left(0, 2 \right)
  \epsilon_{i} &\sim \mathrm{Normal}\left(0, \sigma_{\epsilon} \right)
  \sigma_{\epsilon} &\sim \mathrm{Normal}\left(0, 5 \right)
\end{align}

\begin{align}
  a_{1}, \tau, \omega &\sim \mathrm{Normal}\left(0, 10 \right)
  \nu_{d}  &\sim \mathrm{Normal}\left(0, \sigma_{\nu} \right)
  \sigma_{\nu} &\sim \mathrm{Normal}\left(0, 2 \right)
  u_{i} &\sim \mathrm{Normal}\left(0, \sigma_{u} \right)
  \sigma_{u} &\sim \mathrm{Normal}\left(0, 5 \right)
\end{align}
...
One important feature of this dissertation is that our causal variable of interest, the policy ideology of district-party publics, is not measured with certainty.
The model in Chapter&nbsp;\@ref(ch:model) estimates this quantity up to a posterior distribution, but the variation in the posterior distribution creates an additional layer of uncertainty in our understanding of causal effects. 
This creates theoretical issues with the definition of treatment effects as well as practical issues for estimation, which I describe in turn.


As discussed in Chapter&nbsp;\@ref(ch:causality), it is natural to interpret causal inference as Bayesian updating about a causal parameter using models and data.
Following @rubin:1978:bayesian's Bayesian interpretation of his own work with potential outcomes, causal inference is a method for generating counterfactual estimates of an outcome variable as if they were predictive draws from the posterior distribution.
Supposing that we seek the causal effect of an intervention on group ideology, $\tau$, and we observe only one potential outcome, $y_{g}(\bar{\theta}_{g})$, how do we specify the posterior distribution if we set group ideology to some other value $\theta'$?
Not only do we have posterior uncertainty about $\tau$, but we are also uncertain about the actual value of $\bar{\theta}_{g}$ that we observed in the first place, since the measurement model from Chapter&nbsp;\@ref(ch:model) estimates it only up to a posterior distribution. 
Our posterior distribution for missing potential outcomes reflects both sources of uncertainty. 
Econometrics refers to this problem as "errors in variables," but it takes on a special meaning when it comes to the interpretation of causal interventions, since it stresses our understanding of what an "observed" potential outcome is.
