...
## Empirical Approach: Bayesian Sequential-$g$ 
...
The demediated outcome is then used in the second stage model, which is different from the first stage in two ways.
First, the mediator falls out of the equation because it no longer varies across observations. 
...
Under the identification and statistical assumptions, $\tau$ measures the average controlled direct effect of a one-unit increase in district-party ideology.
More generally, the ACDE of setting district-party ideology to $\bar{\theta}$ is
\begin{align}
  ACDE(\tau, \bar{\theta}, \bat{\theta}') &= \tau \left(\bar{\theta} - \bar{\theta}')
  (\#eq:general-ACDE)
\end{align}
where $\bar{\theta}'$ is a reference value of district-party ideology.
...
This is consistent with the multilevel model setup that we have so far, where the treatment effect is captured using district-level data and district-level parameters only (see Equation \@ref(eq:general-ACDE)).

<!-- 
If group size is correlated with treatment effect, the parameter is not the avg effect. This could be the case is candidates who run unopposed do not feel as strong of effects of treatment. Then again, this is hard to identify with the raw data, since there can be anticipatory effects that enable treatment effects even in the absence of emergent challengers. In this case, you might calculate an average of individual treatment effects by weighting each group treatment effect by its group size, since larger groups apply to more individuals.

If treatment is group-level, the natural treatment effect is at the group level, but you may not get this by averaging individual units if group size is correlated with treatment assignment. 
This can happen if more primary competition has feedback effects on local ideology.
I think I feel pretty comfortable saying that we have no idea if this is even remotely true, but it could be intuitive. 
So you want to average the group effects! [@Hill:2013:multilevel-causal-inf]
-->
