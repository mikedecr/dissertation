...
- take credit for CDE interpretation
...
This chapter investigates the effect of district-party ideology on primary candidate positions using sequential $g$-estimation, a structural modeling approach that estimates the direct effect of district-party ideology while fixing the mediating effect of district-level voting.
...
Because the Republican Party has an ideological underpinning, and elite political actors will be more aware of partisan ideology than individual constituents will be, Republican candidates may not place great importance on positioning themselves to cater to non-conservative policy views held by their constituents.
Democrats, meanwhile, may appear more responsive to district-party ideology because local opinion variation reflects the social group profile of the district that Democratic candidates in each district must represent. These "asymmetric party" predictions diverge from [@clinton:2006:constituent-representation], who finds that Republicans are more responsive to within-party opinion variation than Democrats but doesn't provide much theoretical exploration of why that would be so.
...
```{r plot-rules-means, include = TRUE, out.width = "100%", fig.width = 8, fig.height = 3, fig.cap = "Average CF scores candidate positioning in states with closed, semi-closed, and open primary rules.", fig.scap = "Average CF scores in states with closed-semi-closed, and open primary rules."}
```
...
Furthremore, intervening on the location of the _district_ has no necessary effect on the party location.
...
\begin{align}
  ...
  \E{\text{CF}_{i}\left(\theta, \bar{v} = 0.5 \right) - \text{CF}_{i}\left(\theta, \bar{v} = 0.5 \right) \mid X_{d} = x}
...
\begin{split}
  ...
  E{b\left(\text{CF}\right)_{i}}  &= E\{b\left(\text{CF}\right)_{i}\left(\theta, V_{d} = \bar{v}\right)}
\end{split}
(\#eq:po-demediation)
\end{align}


```{r plot-g-dag, include = TRUE, fig.width = 9, fig.height = 10, out.width = "100%", fig.cap = "Causal graphs describing the modeling problem and sequential $g$ estimation. The stage 1 graph identifies the effect of the past district voting ($V_{d}$) on candidate positioning ($\\text{CF}_{i}$). The stage 2 treatment-outcome model subtracts the district vote effect from candidate positions and identifies the effect of district-party ideology $\\bar{\\theta}_{g}$ on the demediated CF score $b(\\text{CF})_{i}$, which is equivalent to the controlled direct effect on the raw CF score. The final graph shows where unadjusted confounders violate the nonparametric causal identification assumptions in stage 1 ($U1$) and in stage 2 ($U2$)."}
```
...
\begin{split}
  \E{\text{CF}_{i}\left(\theta, \bar{v}\right) - 
     \text{CF}_{i}\left(\theta', \bar{v}\right) 
     \mid X_{d} = x 
   } \\
    \qquad =
    \E{b\left(\text{CF}\right)_{i}\left(\theta\right) - 
       b\left(\text{CF}\right)_{i}\left(\theta'\right) 
       \mid X_{d} = x 
    } \\
   \qquad =
    \E{b\left(\text{CF}\right)_{i} \mid \bar{\theta}_{g} = \theta, x} - 
    \E{b\left(\text{CF}\right)_{i} \mid \bar{\theta}_{g} = \theta', x}
\end{split}
(\#eq:po-stage-2)
\end{align}
...
\begin{align}
  \p{\tau \left(\bar{\theta} - \bar{\theta}'\right) \mid \mathbf{CF}} 
  &= 
  \int \p{\tau \left(\bar{\theta} - \bar{\theta}'\right), \boldsymbol{\pi} \mid \mathbf{CF}} \diff \boldsymbol{\pi},
  (\#eq:post-acde)
\end{align}
...
In practice, we use MCMC samples to approximate this posterior distribution by picking any two values of $\bar{\theta}$ and $\bar{\theta}'$, extracting posterior samples for $\tau$, and calculating the ACDE for each MCMC sample iteration. 
...
Uncertainty intervals for the ACDE can be estimated by noting which MCMC samples bound the inner 90 or 95% of posterior samples.
...
- who?
------------------------->
which accomplishes a similar goal as the prior distribution but throws away all prior information about the covariance between ideal points.
<!------- TO DO ---------
- cite?
------------------------->
...
\begin{split}
  \alpha_{d} &\sim \text{Normal}\left(0, \sigma_{\alpha} \right) &\
    \sigma_{\alpha} &\sim \text{Half-Cauchy}\left(0, 1 \right) \\
  \nu_{d}  &\sim \text{Normal}\left(0, \sigma_{\nu} \right) &\
...
\begin{split}
  \epsilon_{i} &\sim \text{Normal}\left(0, \sigma_{\epsilon} \right) &\
    \sigma_{\epsilon} &\sim \text{Half-Cauchy}\left(0, 2 \right) \\
  u_{i} &\sim \text{Normal}\left(0, \sigma_{u} \right) &\
    \sigma_{u} &\sim \text{Half-Cauchy}\left(0, 2 \right)
\end{split}
(\#eq:g-resid-priors)
\end{align}
...
The @boatright-et-al:2017:primary-timing-data data also provide @mayhew:2014:placing-parties's five-level "traditional party organization" and binary "persistent factionalism" classifications for state level [@mayhew:1986:placing-parties].
...
All controls except for the fixed effects and binary indicators from @mayhew:2014:placing-parties are centered at their means and scaled by their standard deviations.
...
- show overall findings (color = party, panels for param sets)
- show cool ideal point results
- show key parameters in incumbency subgroups (don't say too much abt theory)



### Primary rules and "effect modification" {#sec:pos-rules}
...
Theories of primaries and representation:

- The DAG reveals how these things could be confounders.
- if intermediary groups are the key causal story, but intermediaries are sampled from the party voters, then the party voters has an effect.
- if the party voters take cues from intermediaries (which might instead be functions of industry, demographics, etc), then the causal effect is confounded if they are POST controls but have OTHER routes to CF scores.
- draw a dag:
...
Better data, better research design.

- more years, DiD modeling.
- variable treatment timing has created HUGE problems in DiD literature
- Methods for high-quality causal inferences using panel data and fixed effects are extremely new.

Better data, more confounding mechanisms.

- Thermostatic opinion. national politics -> local ideology
