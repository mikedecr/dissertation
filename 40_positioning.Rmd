...
This is mainly because most studies do not explicitly measure the ideological preferences of primary voters, instead using aggregate measures of voting that neither identify policy preferences nor differentiate between partisan constituencies within a district [@kernell:2009:districts].
The IRT measures of district-party ideology that I create in Chapter \@ref(ch:model) are, to my knowledge, the first ideal point scores for district-party groups to be applied to the study of congressional primaries.^[
  For IRT estimates of partisan constituencies at the U.S. state level, see @caughey-et-al:2018:ideological-nationalization.
]
...
Although recent studies of primary candidates tend not to study the overall relationship between voters' policy ideology and candidate positions, there is an enduring interest in the way _primary rules_ shape candidates' positioning incentives through the preferences of the electorate.
Primary rules refer to regulations in state election law that control which voters can participate in primary elections and the freedoms that parties have to set those participation rules within the scope of the law.
...
There are states whose primary rules fall between these two extremes, such as "semi-closed" systems that allow party-unaffiliated voters to choose which party primary they want to vote for, but registered partisans can only vote in the primary of their registered party affiliation.
Political scientists and political observers have speculated that these rules shape candidates' positioning incentives by changing the composition of primary voters.
...
More open systems, especially blanket primary systems where candidates from all parties run for the same finite number of slots on the general election ballot, encourage primary candidates to take more moderate stances to win a primary election among a more ideologically diverse constituency.
The general hypothesis, therefore, is that more restrictive primary rules exacerbate ideological polarization in congress and in state legislatures.
...
- @porter-treul:2020:primary-experience
    ...
    - not an obvious pattern about the ideological extremity of inexperienced candidates
...
- @rogowski-langella:2015:primary-systems
- @ahler-citrin-lenz:2016:CA-open-primaries



primary positioning [@rogowski-langella:2015:primary-systems]

Primary systems
    - redistricting, same district, divergent candidates [@mccarty-poole-rosenthal:2009:gerrymandering; @brunell-grofman-merrill:2016:polarization-components]

- not everything: miller and stokes
- other things: 
    - partisan organization [ @mcclosky-et-al:1960:conventions; @layman-et-al:2010:activists-conflict-extension]
    - party network
...
- Researchers find evidence that candidate positioning is related to district-level aggregate voting (e.g.\ the presidential vote) and to voter preferences [@ansolabehere-et-al:2001:candidate-positioning; @clinton:2006:constituent-representation; @ansolabehere-et-al:2008:voter-ideology].
...
party_fits <- full_data %>%
  ...
  mutate(
    ...
    lmfit = map(
      .x = data,
      .f = ~ lm(recipient_cfscore_dyn ~ theta_mean_raw, data = .x)
    ),
    lmtidy = map(lmfit, tidy, conf.int = TRUE),
...
```{r plot-by-party, include = TRUE, fig.width = 8, fig.height = 5, fig.cap = "Topline relationship between district-party ideology and candidate positions. The horizontal axis plots the posterior mean for a group ideology, and the vertical axis is the dynamic CF score for primary candidates included in the DIME congressional candidate database.", fig.scap = "Topline relationship between district-party ideology and candidate positions."}
```
...
```{r plot-by-incumbency, include = TRUE, out.width = "100%", fig.width = 8, fig.height = 4, fig.cap = "District-party ideology and candidate positioning across candidate incumbency status.", fig.scap = "District-party ideology and candidate positioning for incumbents, challengers, and open-seat candidates."}
```
...
```{r plot-by-cycle, include = TRUE, out.width = "100%", fig.width = 8, fig.height = 4, fig.cap = "District-party ideology and candidate positioning across election cycles within the 2010s districting cycle.", fig.scap = "District-party ideology and candidate positioning in 2012, 2014, and 2016."}
```
...
full_data %>%
  filter(
    ...
    is.na(theta_mean_raw) == FALSE
  ) %>%
  ggplot() +
  aes(x = scale(theta_mean_raw), y = recipient_cfscore_dyn) +
  facet_grid(
...
These variances are themselves given half-Normal priors with weakly informative scale parameter values that are about half the range of the raw outcome data within each party.
\begin{align}
\begin{split}
  \alpha_{d} &\sim \text{Normal}\left(0, \sigma_{\alpha} \right) &
    \sigma_{\alpha} &\sim \text{Half-Normal}\left(0, 2 \right) \\
  \nu_{d}  &\sim \text{Normal}\left(0, \sigma_{\nu} \right) &
    \sigma_{\nu} &\sim \text{Normal}\left(0, 2 \right)
\end{split}
(\#eq:g-ranef-priors)
\end{align}
And finally, each stage of the model has a Normal error term for candidates within districts. 
The variances for these errors are given half-Normal priors with wider scale values that the districts errors, since residual variation between any two candidates is likely larger than the variation between average candidates in any two districts.
\begin{align}
\begin{split}
  \epsilon_{i} &\sim \text{Normal}\left(0, \sigma_{\epsilon} \right) &
    \sigma_{\epsilon} &\sim \text{Half-Normal}\left(0, 5 \right) \\
  u_{i} &\sim \text{Normal}\left(0, \sigma_{u} \right) &
    \sigma_{u} &\sim \text{Normal}\left(0, 5 \right)
\end{split}
(\#eq:g-resid-priors)
\end{align}
...
Errors from smaller districts borrow more information from the overall distribution of districts, downweighting the contributions of smaller districts by shrinking their errors toward a mean of zero.
This has a similar intuition as a weighted least squares regression on the district-averaged data, where districts with more candidates are more informative and receive greater weight.
Because the multilevel model is Bayesian, however, the model can handle single-candidate districts by drawing district and candidate error terms from their respective prior distributions. 
This is yet another example where priors stabilize pathological model behavior, and underscores how the flexibility afforded by Bayesian model-building allows us to confront the idiosyncrasies of a dataset with tactics that are both intuitive and feasible.
...
### Model parametrization in Stan

- identifiability of district random effects
- marginalize over the random effects
- this retains the added variance, but without the need to explicitly sample random effects, which creates identifiability problems
- variance-covariance matrix is like so
- precision is better
- 
