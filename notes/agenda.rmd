...
- Ask Alex about: 
    ...
    - [ ] identifiability in heteroskedastic model (Londregan?)
- Paper fixes
    - [ ] cite bailey for ranef approach (incidental parameters problem)
    - [ ] vector form covariates in model math section
    - [ ] pi-bar in the group model (to show that $\neq \pi$ )
    - [x] IRT model from formal assumptions
    ...
    - [x] Document-level aesthetic to-dos (some time...)
    - [ ] first references: MCMC, IRT, SPD
- Code fixes
    - [ ] Fix linstat runs
        - [ ] save stderr and stdout separately
        - [ ] run short background
        - [ ] mac access to linstat files
        - [ ] run long background
    - [ ] discrimination should just be mean 0 on the log scale, not product 1
    ...
    - [ ] fix html/latex specific cache issue (I hate waiting)
    - [ ] log1m/log1p functions for probit link
