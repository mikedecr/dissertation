...
- "moderate" districts are internally polarized
- formal model:
  + less information about the location of the median voter
  + weaker incentive to offer moderate policy
- empirically:
...
Do primary challenges increase when the incumbent performed weakly in the previous cycle

- Clinton Republicans in California

Unified model of Congress

- dv: vote in November
- iv: district mean 
- national environment plus district offsets
- national environment is latent
  + generic ballot
  + special elections
...
- multinomial logit
- $Pr(y_ij = k) = XB_{k} / sum_k(XB_k)$


Cross-validation:

- maybe not respecting the item parameters?



Language:

- "First, it is not ambitious to expect an estimate based on 20 to 30 policy questions to have some kind of correlation with an issue as polarizing as presidential vote choice."
  + that covers me a little bit
  + even if not good at predicting the very next issue, should still be related to voting, no?


 

concerns with my own model:

- groups mean way fewer parameters! We don't estimate individuals, we just assume that they're normal draws on the linear index
- check: 
  + extremism -> lower levels of presidential defection?
  + lower levels of punishment for extreme candidates? (presuming that the Hall/Thompson effect is what's happening, depressing within-party turnout)
  + outliers could be making relationships stronger even if most don't matter