...
tidy_homsk <- 
  ...
  mutate( 
    tidy = map(
      conf, ~ tidy(mcmc_homsk, conf.int = TRUE, conf.level = .x, rhat = TRUE, ess = TRUE)
    ),
    tidy = ifelse(
      test = conf == 0.5, 
      yes = map(tidy, 
...
theta_draws <- 
  spread_draws(mcmc_homsk, theta[group], n = 100) %>%
  # group_by(.draw) %>%
  # nest() %>%
  # sample_n(100) %>%
  # unnest() %>%
...
# the newnew, Jan 2020, gotta figure out how to work this in with the rest
thetas %>%
  pivot_wider(
    names_from = party, names_prefix = "estimate",
...
theta_scatter_means <- thetas %>%
  ...
  nest() %>%
  ungroup() %>%
  pivot_wider(names_from = party, values_from = data) %>%
  # spread(key = party, value = data) %>%
  unnest(cols = c(`1`, `2`), .name_repair = "minimal") %>%
  print()
