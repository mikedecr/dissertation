...
parameters {
  ...
  // constraints: max bias and fractions of biases
  real max_bias_select; 
  real max_bias_outcome; 
  simplex[nodes_select] bias_slice_select;
  simplex[nodes_outcome] bias_slice_outcome; 
...
transformed parameters {
  ...
  // constrainted weights
  matrix[P_select, nodes_select] hid_select = hid_select_raw;
  matrix[P_outcome, nodes_outcome] hid_outcome = hid_outcome_raw;
  // matrix[P_select, nodes_select] hid_outcome;

  // biases are increasing over nodes.
  for (ns in 1:nodes_select) {
      hid_select[1, ns] = 
        max_bias_select * sum(bias_slice_select[1:ns]);
  }

  for (no in 1:nodes_outcome) {
      hid_outcome[1, no] = 
        max_bias_outcome * sum(bias_slice_outcome[1:no]);
  }
...
model {
  ...
  // ----- constraints -----
  max_bias_select ~ normal(0, 1); // could be LKJ
  max_bias_outcome ~ normal(0, 1);
  
  // slice simplexes are flat
  bias_slice_select ~ dirichlet(rep_vector(1, nodes_select));
  bias_slice_outcome ~ dirichlet(rep_vector(1, nodes_outcome));
