# Measuring District-Party Ideology with a Group IRT Model  {#ch:model}
...
This chapter proceeds in three major steps. First, I review the theoretical basis for ideal point models, connecting formal models of policy choice to statistical models for estimating latent attitudes from data.
Second, I specify and test a group-level model for estimating district-party public ideology.
This discussion includes details that are relevant to Bayesian estimation, including identification restrictions on the latent policy space, specification and simulation of prior distributions, and model parameterizations that facilitate estimation with Markov chain Monte Carlo (MCMC).
Lastly, I describe how I fit the model to real data and review the results.
...
Spatial theories invoke a concept of "policy space," where political preferences and policy outcomes are represented as locations along a number line.
The key policy space in this project is a left–right ideological continuum where progressive or "liberal" policies occupy locations on the left side of the continuum, while conservative policies are on the right side.
Actors are at least partially motivated by their policy preferences, so they strive to achieve policy outcomes that are closest to their own locations in policy space, holding other factors constant.
...
Formal models like these encode assumptions that structure policy choice: whether the policy space is one-dimensional or multidimensional, the functional form of the Actor's policy utility, and distributional assumptions about error terms in the utility function.
...
The formal model of ideal points is distinct from a statistical model to estimate ideal points.
The formal model is a theoretical exercise; it establishes the incentives and likely decisions that Actors make in specific choice contexts, building theoretical intuitions that can be tested in empirical studies of real-world politics.
The statistical model explicitly or implicitly assumes an underlying formal model and estimates its parameters using data.
The statistical model in this chapter begins with a formal model where actors represent partisan voters, choices are statements of policy preferences in public opinion surveys, and ideal points are summarized on a single policy dimension.^[
  ...
  Past work on the strategic positioning dilemma theory guides our choice of a single-dimensional model because the theory holds that candidates take more progressive or conservative positions to target the policy preferences of their voters.
]
An Actor $i \in \{1, \ldots, n\}$ has an ideal point in policy space represented by $\theta_{i}$. 
The Actor is confronted with survey item $j \in \{1, \ldots, J\}$ and chooses either the Left alternative located at position $L_{j}$ or a Right alternative located at $R_{j}$. 

The Actor's choice is a function of the distance between their ideal point and the respective choice locations.
The Actor maximizes their utility if they can choose a policy located at exactly their ideal point, and they lose utility for policy choices that are farther from their ideal point.
...
\begin{align}
  ...
  (\#eq:utility-fns)
\end{align}
where $u^{\mathtt{R}}_{ij}$ and $u^{\mathtt{L}}_{ij}$ are idiosyncratic error terms for the Right and Left alternatives, respectively.
...
Let $y_{ij} = 1$ indicate that the Actor chooses Right and $y_{ij} = 0$ indicate that they choose Left.
...
\begin{align}
  ...
  (\#eq:choice-iff)
\end{align}
To visualize this choice, I plot the deterministic components of \@ref(eq:utility-fns) in Figure \@ref(fig:plot-utility), omitting the stochastic utility terms. 
...
In the current example, the Actor is closer to Right than to Left, so they receive greater utility (or lose _less_ utility) by choosing Right instead of Left.
...
Supposing that these idiosyncratic terms follow some probability distribution, the choice can be represented probabilistically:
\begin{align}
  \begin{split}
...
By expanding the polynomial terms and then factoring...
\begin{align}
\begin{split}
  \mathrm{Pr}\left(y_{ij} = 1\right) &= 
    \mathrm{Pr}\left(
...
Midpoints are significant for spatial models because they represent the threshold where the Actor is indifferent between policy alternatives.^[
...
The term $\theta_{i} - \frac{R_{j} + L_{j}}{2}$ conveys which policy alternative is closer to the Actor. 
The expression is positive if the Actor is closer to Right and negative if closer to Left.
Second, $2\left(R_{j} - L_{j}\right)$ measures how far apart the policy alternatives are from one another.
Together, these two terms capture whether the Actor is closer to the Left or Right policy and by how much.
...
Similar to Equation \@ref(eq:factor-item-params), $\theta_{i} - \kappa_{j}$ shows how far the Actor is from the midpoint between Left and Right and in which direction. 
The discrimination parameter $\iota_{j}$ behaves as a slope or elasticity on this distance: the distance from the midpoint has a stronger influence on the Actor's choice when the policy alternatives are farther from each other because utility is a quadratic function of the ideological distance.
I explore the intuitions and consequences of this functional form more thoroughly in Section \@ref(sec:irt).

A complete statistical model is obtained by a distributional assumption for $\epsilon_{ij}$.
A common assumption in ideal point modeling is to assume that $\epsilon_{ij}$ is Normal, which assumes that utility error represents a sum of unrelated utility shocks.^[
  Many IRT models assume that $\epsilon_{ij}$ follows a Logistic distribution, resulting in a logit model [for example @londregan:1999:ideal-pts].
  The logit model is computationally convenient, but the Logistic error term is harder to justify under spatial theory. 
  Furthermore, the Normal error term better facilitates the group-level extension below.
]
...
The probit model simply states that $i$ prefers Right over Left as a function of ideal point distance plus Normal error.


## Item Response Theory {#sec:irt}

Ideal point models have a similar construction to models developed under item response theory (IRT), a modeling framework from psychometrics.
...
The canonical psychometric example is educational testing, where several test questions are used to measure a student's academic "ability" level. 
...
An important point about measurement models is that they require assumptions to reconstruct a signal about latent traits from observed data, otherwise no inferences can be drawn.
In this sense, the estimates are always sensitive to the model's assumptions.
While this is always important to acknowledge, it is an ever-present consideration even for non-IRT measurement strategies, such as additive indices that estimate ideology by summing or averaging responses to a battery of policy questions.
Additive indices are special cases of measurement models where key parameters are assumed to be known with certainty and fixed across all policy items, which is problematic if there is any reason to suspect that item responses are correlated across individuals.^[
...
ex_item_params <- 
  bind_rows(
    .id = "model",
    'No Item Variation' = item_params_0p,
  ...
  mutate(
    item = 1:n(),
    model = fct_relevel(
      model, 'No Item Variation', "Varying Item Difficulty"
    )
  ) %>%
  print()
...
Consider a simple model where a student $i$ is more likely to answer test questions $j$ correctly if she has a greater academic ability level $\theta_{i}$. Analogously, a citizen who is more conservative is more likely to express conservative preferences for policy question $j$. 
Using the probit setup from above, we can represent this simple model with the equation:
\begin{align}
  \mathrm{Pr}\left( y_{ij} = 1 \right) &= 
...
Even so, political science is replete with measurement approaches that omit all item-level variation, such as the racial resentment scale, the additive index of political participation, and more [@henry-sears:2002:symbolic-racism; @verba-nie:1972:participation].

Item response theory assumes that items reveal systematically different information from one another and models these item characteristics using _item parameters_.
...
\begin{align}
  ...
  (\#eq:1p-ogive)
\end{align}
IRT parlance refers to $\kappa_{j}$ as the item "difficulty" parameter.
...
The probability of a Left or Right choice depends on the Actor's ideal point location relative to the midpoint.
...
In the ideal point context, it captures how strongly a policy question divides the responses of progressive and conservative Actors.

```{r plot-example-iccs, include = TRUE, echo = FALSE, fig.scap = "Examples of item characteristic curves under different item parameter assumptions.", fig.cap = "Examples of item characteristic curves under different item parameter assumptions: ability-only (left), one parameter (middle), and two parameter (right).", fig.width = 9, fig.height = 4, out.width = "100%"}
ggplot(ex_iccs) +
  aes(x = theta, y = prob) +
  facet_wrap(~ model) +
...
Each panel shows how the subject's ability/conservatism (the horizontal axis) affects the probability of a correct/conservative response (the vertical axis), where the shape of the curve is set by the values of the item parameters.
These curves are commonly referred to as *item characteristic curves* (ICCs) or *item response functions* (IRFs).
The leftmost panel shows a model with no item effects whatsoever, so response probabilities are affected only by the latent trait.
The middle panel shows a one-parameter model where item difficulties/cutpoints are allowed to vary.
Difficulty parameters behave as intercept shifts, so they change the value of $\theta$ where a subject is indifferent between responses.
...
If $\theta_{i} - \kappa_{j} = 0$, the item cutpoint falls directly on an Actor's ideal point, so the Actor is ideologically indifferent between Left and Right and chooses Right with probability $0.5$. 
The value of $\kappa_{j}$ increases by moving either the Right or Left alternatives to the right (increasing $R_{j}$ or $L_{j}$), subject to the constraint that $R_{j} > L_{j}$. 
Larger values of the item cutpoint imply a lower probability that the Actor chooses Right, since $\kappa_{j}$ has a negative effect on the conservative response probability.^[
...
The opposite intuition holds as the Left position becomes increasingly progressive, resulting in lower values of $\kappa_{j}$ that imply a higher probability of choosing Right, all else equal.
...
It connects the individual- and group-level response models and lays out a hierarchical prior for smoothing district-party ideal points.
The group-level model is similar to the approach laid out by @caughey-warshaw:2015:DGIRT, but differs in important ways that I describe as I go.
I also discuss several technical features for estimating the model, including the structure of the data, model parameterization, identifiability restrictions, and prior distributions for Bayesian estimation.
...
This parameterization replaces item "discrimination" with item "dispersion" using the parameter $\sigma_{j} = \iota^{-1}_{j}$, which controls the additional "measurement error" introduced by a survey question beyond the standard Normal utility error in the probit model.
\begin{align}
\begin{split}
  y_{ij} &\sim \mathrm{Bernoulli}(\pi_{ij}) \\
  \pi_{ij} &= \Phi\left(\frac{\theta_{i} - \kappa_{j}}{\sigma_{j}}\right).
\end{split}
(\#eq:y-i-probit)
\end{align}
The group model assumes that individual ideal points are Normally distributed within a group.
In other words, individual ideal points deviate from the mean ideal point in the group as a result of a sum of random forces.
\begin{align}
  \theta_{i} &\sim 
...
where $\bar{\theta}_{g[i]}$ and $\sigma_{g[i]}$ are the mean and standard deviation of ideal points within $i$'s group $g$.^[
  Notation for Normal distributions will always describe the scale in terms of standard deviation $\sigma$ instead of variance $\sigma^2$.
] 
...
Instead, we aggregate individual-level responses to the group level and estimate the group ideal point by marginalizing over the individuals within the group.
...
If trials are conducted independently across groups and items (an assumption that is relaxed later), we can model the grouped outcome as a binomial random variable,
\begin{align}
\begin{split}
...
Our uncertainty about the item response now contains two sources of variance: the uncertainty introduced by the item itself, $\sigma_{j}$, and the variance of the individual ideal points around the group mean, $\sigma_{g}$.
...
The group model described so far can be estimated straightforwardly if there are enough responses from enough individuals in each district-party groups. In practice, it would take dozens more surveys than are practically available to achieve this level of precision with survey data alone.
Instead, I build a hierarchical model for the group parameters that regularizes the group ideal points in a principled way.
...
This section describes the multilevel structure using traditional notation for hierarchical models; Section \@ref(sec:noncentering) describes how I reparameterize the model for computational implementation.
...
I represent this notationally by referring to group $g$'s district as $d[g]$ and $g$'s party is $p[g]$. 
...
\begin{align}
  ...
  (\#eq:theta-g-hlm)
\end{align}
where $\mu_{p}$ is a constant specific to party $p$, and $\mathbf{x}_{d}$ is a vector of congressional district-level covariates with party-specific coefficients $\beta_{p}$.
...
This is a profound departure from the structure laid out by @caughey-warshaw:2015:DGIRT, who estimate the same set of geographic effects for all groups in the data.
...
My results suggest that many of the contextual factors in the model have different effects in different parties (Section \@ref(sec:ideal-covariates)).
<!------- TO DO ---------
- Milk this _MORE_???
------------------------->

The state offsets are regressed on state features as well,
\begin{align}
...
Each state effect contains a party-specific region effect $\alpha^{\mathtt{region}}_{rp}$ for Census regions indexed $r$, which is a modeled mean-zero effect to capture region-level correlations across the state effects.
\begin{align}
...
I return to the discussion of priors and model parameterization below.
...
Researchers wishing to extend the district-party ideal point model could construct a dynamic model that scales district-party groups across election cycles or across 10-year districting cycles.
Because congressional districts change boundaries between districting cycles—sometimes falling out of or coming into existence, depending on decennial apportionment of districts to states—the district-party groups may not be considered as the "same group over time."
The ideal point space, however, can be identified over time either by specifying dynamic priors on hierarchical regression parameters or by fixing item parameters over time [@caughey-warshaw:2015:DGIRT].

### Identifying the latent policy space {#sec:identification-restrictions}

Ideal point models, as with all latent variable models, are unidentified without restrictions on the latent space.
Without these restrictions, the likelihood model as written can rationalize an infinite number of parameter estimates with no basis for deciding which estimates are best, even with infinite data.
...
All statistical models have the potential to be non-identified in this way, but covariate data typically provide the restrictions necessary to identify a model.^[
  We could imagine shifting, stretching, or reversing the sign of a covariate to reveal the same identifiability issues.
]
...
The polarity of the space is fixed by coding all survey items such that conservative responses are $1$ and progressive responses are $0$.
This implies a restriction that all discrimination parameters are positive, 
which ensures that moving an ideal point to the _right_ in the latent ideological space always leads to an increased probability of a _conservative_ item response, all else equal.
...
Restricting the sum of the cutpoints to zero implies that the mean of the cutpoints is also zero.

Lastly, I restrict the scale of the latent space by restricting the product of the $J$ discrimination parameters to be equal to $1$.
Unlike the @caughey-warshaw:2015:DGIRT approach, I implement this constraint on the log scale by restricting the log discrimination parameters to sum to $0$, which achieves an equivalent transformation.
Performing this restriction on the log scale facilitates a unique prior specification for the item parameters that I describe in Section \@ref(sec:item-priors).
...
The group-level model learns about group ideal points by surveying individuals within groups, but the model currently assumes that all $y_{ij}$ are independent conditional on the item.
...
\begin{align}
  s^{*}_{gj} &= n^{*}_{gj}\bar{y}^{*}_{gj}
  (\#eq:wted-s)
\end{align}
which should not exceed the adjusted sample size.^[
  Statistical analysis code implements the weighted number of successes as the minimum between $n^{*}_{gj}\bar{y}^{*}_{gj}$ and $n^{*}_{gj}$, to ensure that the weighted successes do not exceed the adjusted sample size.
  By the construction of $n^{*}_{gj}$ and $y^{*}_{gj}$, however, in practice there should be no instances where the weighted successes can exceed the adjusted sample size.
]

It is likely that many values of $n^{*}_{gj}$ and $s^{*}_{gj}$ will be non-integers. 
This presents a problem for modeling a Binomial random variable with statistical programs, which expect integer-valued numbers of successes and trials.
To rectify this, I follow @caughey-warshaw:2015:DGIRT and round the weighted number of trials up (to ensure no trial counts of zero) and round the weighted number of successes to the nearest integer. 
These integer results are then used in the IRT likelihood as follows.^[
  Another possible tactic would be to use a quasi-likelihood approach [@ghitza-gelman:2013:subgroup-mrp], supplying the weighted number of successes and trials to the Binomial probability mass function directly instead of using a built-in computer function that fails for non-integers.
  This is more complex to implement in Bayesian estimation software because it requires the researcher to add log probability expressions directly to a construct in a Stan program called the "log density accumulator" [@carpenter-at-al:2016:stan], though other research has demonstrated the routine for a Binomial quasi-likelihood problem [@decrescenzo-mayer:2019:voter-ID, 350].
]
\begin{align}
...
I estimate the model using Stan, a probabilistic programming language for defining Bayesian models that are estimated using `C++` [@carpenter-at-al:2016:stan].
...
Although these Hamiltonian mechanics are highly effective for fast and reliable Bayesian estimation, the model must be parameterized to ensure that the geometry of the posterior distribution is favorable to the proposal trajectories.
...
At a broader level, there are simply no reference texts for contemporary Bayesian analysis in political science that reflect recent innovations in Hamiltonian Monte Carlo methods.
This is despite the fact that HMC has been the state of the art MCMC method for most applied Bayesian problems for several years [@patil-et-al:2010:PyMC;@carpenter-at-al:2016:stan; @betancourt:2017:conceptual-hamiltonian; @burkner:2017:brms].^[
...
The most essential elements of the model to reparameterize are the hierarchical priors.
The group-level IRT model regularizes parameters estimates using several hierarchical regressions: group ideal points are regularized with a regression on district-level data, state effects are regularized with a regression on state-level data, and region effects are regularized using a mean-zero hierarchical prior.
Hierarchical models like these have posterior distributions whose curvatures are known to present difficulties for sampling algorithms [@betancourt:2015:hamiltonian; @papaspiliopoulos-et-al:2007:parameterizations].
To improve the estimation in Stan, the best practice for most situations is to parameterize hierarchical models using a "non-centered" parameterization rather than a "centered" parameterization.
...
the non-centered parameterization defines $\bar{\theta}_{g}$ as a function of covariates and a random error term,
\begin{align}
  \bar{\theta}_{g} &=
...
\end{align}
where $u_{dp}\tau_{p}$ behaves as a group-level error. 
The group-level error term is decomposed into a standard Normal variate $u_{dp}$ and a scalar parameter $\tau_{p}$ that controls the variance of the error term.
The scale parameter $\tau_{p}$ is then given its own prior.

The non-centered model is algebraically equivalent to centered model in the likelihood, but the non-centered parameterization "unnests" the mean and the scale from the hierarchical distribution.
This parameterization improves MCMC sampling by de-correlating random variables in the posterior distribution, leading to more efficient MCMC exploration. 
...
Equation \@ref(eq:theta-g-noncenter) is an incomplete implementation of the non-centered form.
To complete the parameterization, I apply it to all hierarchical components in the regression, including the state and region effects,
\begin{align}
\begin{split}
  \bar{\theta}_{g} &=
    \mu_{p} + 
  ...
  u_{sp}^{\mathtt{district}}, u_{sp}^{\mathtt{state}}, u_{sp}^{\mathtt{region}} &\sim 
    \text{Normal}\left(0, 1\right) \\ 
  \tau_{p}^{\mathtt{district}}, \tau_{p}^{\mathtt{state}}, \tau_{p}^{\mathtt{region}} &\sim
    \ldots
\end{split}
(\#eq:full-noncenter)
\end{align}
where $u^{\mathtt{district}}$, $u^{\mathtt{state}}$, and $u^{\mathtt{region}}$ are all standard Normal variables, and the scale components $\tau$ for each level have their own prior distributions that I specify below.
...
The primary benefit is the ability to encode external information into a model, allowing the researcher to downweight unreasonable estimates, regularize against overfitting, stabilize weakly identified quantities, and pool estimates across many groups of data [@gelman-hill:2006:multilevel]. 
...
The drawback of Bayesian modeling is that prior specification is additional, challenging work for the researcher.
This section describes and justifies the priors I use in the group ideal point model. 
The discussion here is more detailed than a typical paper describing a Bayesian ideal point model for a few reasons. 
First, the norms surrounding Bayesian analysis are evolving toward more rigorous checking of prior distributions and their implications using prior simulations and other graphical model checks [@betancourt:2018:workflow-blog; @gabry-et-al:2019:visualization].
...
In a few instances, I specify priors that depart from the priors used in previous Bayesian ideal point work for theoretical and practical reasons that I detail below.

As a general orientation, this model employs "weakly informative priors" [@gelman-et-al:2017:prior-likelihood]: priors that use structural information about a model to rule out extreme model configurations.
Weakly informative priors are stronger than flat priors, which can have problematic implications especially for probit and other nonlinear models.^[
  For more on flat priors, see Chapter \@ref(ch:causality).
]
At the same time, weakly informative priors are weaker than fully informative priors that represent substantive beliefs about model parameters.
Weakly informative priors are mainly pragmatic devices that scale parameters to match the order of the variation in the data.
...
One important source of pragmatic prior information is the probit model itself.
Probit models translate latent scale quantities into probabilities using the standard Normal cumulative distribution function.
The latent scale represents $z$-scores, or Normal quantile values, for predicted success probabilities.
This relationship between the quantile and probability scales means that researchers can anticipate the neighborhood of quantile-scale values based on approximate prior information on the probability scale.
For most social science problems, reasonable probability values (between $0.01$ and $0.99$) map to a narrow region of the Normal quantile values.
This is principle generalizes to other nonlinear models with latent scales; even though latent scales are often described as "uninterpretable," the researcher usually needs only a few heuristic rules to derive sensible priors for latent-scale parameters.
...
For instance, if a model quantile value has a $\text{Normal}\left(0, 1\right)$ distribution, this implies a flat distribution over the probability scale.
Priors of this sort are a simple example of weak information: enough regularization to downweight extreme model configurations, but weak enough that the data adjudicate between model configurations in the more plausible regions of parameter space.
I give constants $\text{Normal}\left(0, 1\right)$ priors.
The hierarchical scale parameters in the group, state, and region random effects (denoted $\tau_{sp}$) are given $\text{Half-Normal}\left(0, 1\right)$ priors, and regression coefficients are regularized slightly more with $\text{Normal}\left(0, 0.5\right)$ priors.
Because the covariate data for the regressions are all scaled to be standard deviation $1$, a prior scale of $0.5$ means that approximately 95% of prior mass is concentrated on effect sizes of less than $1$ on the quantile scale.^[
  Normal priors for coefficients are similar to a ridge penalty [@bishop:2006:pattern-rec], a commonplace regularization choice in predictive modeling.
]
...
The widespread understanding that flatter priors always represent more principled prior ignorance is not generally true.
Instead, the correspondence between prior "flatness" and prior "informativeness" is contingent on the scale of the parameter space—an important point that I explore further in Chapter \@ref(ch:causality).
This is especially relevant for models that include nonlinear transformations that stretch and compress different neighborhoods of parameter space.
Early ideal point models for the mass public [e.g. @treier-hillygus:2009:ideology] tend to use highly diffuse priors, such as $\text{Normal}\left(0, \sqrt{1000}\right)$, which place the majority of prior probability on model configurations that are virtually impossible to be true.
More recent Bayesian ideal point models [e.g. @tausanovitch-warshaw:2013:constituent-prefs; @caughey-warshaw:2015:DGIRT] use more reasonable prior scales such as $5$ or $2.5$, which are still broad but at least on an order of magnitude that fits the probit model.


### Priors for item parameters {#sec:item-priors}
...
The discrimination parameter, which has a product of $1$ when scaled, is logged so that it has a sum of $0$ on the log scale. 
...
The correlation matrix $\boldsymbol{S}$ gets a prior from the LKJ distribution, which is a generalization of the Beta distribution defined over the space of symmetric, positive-definite, unit-diagonal matrices [@LKJ:2009:correlation-matrices].
The LKJ prior is an increasingly popular approach to covariance matrix priors that provides more flexibility that inverse-Wishart distributions.^[
  ...
  Conjugacy provides no computational benefit for Hamiltonian Monte Carlo samplers, which frees the researcher to consider more flexible priors that sever the dependence between prior scales and prior correlations [@alvarez-niemi-simpson:2014:covariance-prior; @akinc-vandebroek:2018:covariance-prior].
]
\begin{align}
  \sigma_{\tilde{\kappa}}, \sigma_{\tilde{\iota}} &\sim \text{Half-Normal}\left(0, 1\right) \\
  \boldsymbol{S} &\sim \mathrm{LKJ}(\eta = 2)
\end{align}
The LKJ distribution has one shape parameter $\eta$ that controls the regularization of the off-diagonal correlations.
The value of $\eta$ has a similar intuition to a Beta distribution:
$\eta = 1$ produces a flat prior over correlation matrices, and greater values of $\eta > 1$ concentrate the prior for $\rho$ toward a mode of $0$. 
...
```{r plot-lkj, include = TRUE, fig.height = 9, fig.width = 7, out.width = "70%", fig.scap = "Joint prior for item parameters.", fig.cap = "Joint hierarchical prior for the unscaled item parameters. Top panels show the components of the covariance prior using the separation strategy: scales (left) and LKJ correlation prior (right). Bottom panel shows simulated draws for unscaled item parameters."}
```
