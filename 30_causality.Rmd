...
Priors are "beliefs" in a pragmatic sense: they are "belief functions" in that the height of the prior represents the degree of epistemic support for some parameter value.
The actual degree of a researcher's sincere belief in any prior is essentially zero—priors are about as believable that residuals are exactly Normally distributed or that functional forms are exactly linear [@gelman-shalizi:2013:bayes-philosophy, p. 19].
These assumptions are represent reasonable approximations that structure a model in such a way to efficiently extract information from data.
As such, it is common for Bayesian statisticians to care about the frequency properties of their estimators, designing noninformative priors for optimal learning from data, [@rubin:1984:bayes-frequency; @berger:2006:objective-bayes; @fienberg:2006:objective-bayes-comments], and to engage in workflow practices that "fall outside the scope of Bayesian theory" such as model checking with diagnostic graphics [@gelman-shalizi:2013:bayes-philosophy; @betancourt:2018:workflow-blog; @gabry-et-al:2019:visualization].
...
### Priors are not for de-confounding
