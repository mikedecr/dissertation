...
ran_draws <- 
  ...
  read_rds() %>%
  filter(heterogeneity == max(heterogeneity)) 
```


Lessons:

- default choice implies a strict prior with a limiting-case parameter value
- the parameter is not strongly identified by the data
- choice of prior has a big effect on inferences
- relaxing the prior easily changes the results
...
```{r}
ggplot(signs_direct) +
  aes(x = fct_rev(str_glue("Study {j}")), y = estimate) +
  geom_pointrange(
  ...
  labs(
    x = NULL,
    y = TeX("Estimated effect $\\pm$ 2 std. errors"),
    title = "Yard Sign Effect on Vote Share",
    subtitle = "Field experiments by Green et al. (2016)"
  ) +
  geom_hline(yintercept = 0)
```
...
$\hat{\tau}_{j}$ is the treatment effect estimate in study $j$ with standard error $s_{j}$.

\begin{align}
  \hat{\tau}_{j} &\sim \text{Normal}\left(\mu, s_{j} \right)
  (\#eq:fixed-model)
\end{align}

They compare the meta-analysis estimates for $\mu$ using different priors based on "agnostic," "skeptical," and "optimistic" prior beliefs about $\mu$.
\begin{align}
...
This assumes that there is no heterogeneity across all studies.
This is wrong if...
...
Random model introduces $\tau_{j}$, the study-specific mean.
\begin{align}
  \hat{\tau}_{j} &\sim \text{Normal}\left(\tau_{j}, s_{j} \right)
  (\#eq:random-model)
\end{align}

The study-specific effects are drawn from the population effect.
\begin{align}
  \tau_{j} &\sim \text{Normal}\left(\mu, \sigma)
  (\#eq:random-mean)
\end{align}

The population effect $\mu$ gets the same priors as before, but we've introduced a new parameter, $\sigma$ representing the amount of heterogeneity across studies.
This parameter also needs a prior.
I give it an exponential prior,
\begin{align}
  \sigma &\sim \text{Exponential}(\lambda)
  (\#eq:heterogeneity-prior)
\end{align}
where $\lambda$ is a rate parameter. 
The Exponential prior has an always-decreasing density, so the prior regularizes toward smaller values of $\sigma$.
The expected value of the Exponential prior is $\sigma = \lambda^{-1}$, so we can interpret $\lambda^{-1}$ as an expression of prior heterogeneity across studies.
I estimate the model using different values of $\lambda^{-1}$ to represent different prior assumptions about the level of heterogeneity.
<!-- use data from R -->
...
- but this is also so big as to be useless. Extrapolating Bayes into a philosophy is impractical and frankly dogmatic. It puts the "coherence" of a method above its usefulness.
- And since all models are wrong anyway (Navarro) this is probably taking the whole thing too far.
- We should use Bayes where it's helpful, and criticize Bayes where it isn't helpful (Bayes holes), and acknowledge where the practical implementation of Bayes is helpful even when it violates the supposed "philosophical coherence" of Bayes.
