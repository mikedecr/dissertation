...
Because Bayesian models are joint generative models for parameters and data, the model contains all machinery required to model counterfactual potential outcomes directly.
...
again fixing the treatment status and covariate profile.^[
  Simulations of this sort are possible under any likelihood-based model that posits a generative probability distribution for data given parameters.
  The Bayesian predictive distributions are wider than the likelihood-only distributions, however, since the former integrates over parameters instead of simply conditioning on parameters.
]
...
Denote these counterfactual observations $\tilde{Y}$, which I will now subscript $i$ to show that the model implies a probability distribution for individual data points as well as aggregate treatment effects.
\begin{align}
...
These probability distributions are defined in the prior and after conditioning on observed data, and they can be defined all the way down to the unit level, as long as the Bayesian model contains a unit-level model for the probability distribution of the data given parameters.^[
  Later I will discuss a Bayesian view of models that do not go as far down as the unit-level data model.
  Naturally, those models do not imply direct counterfactual probability distributions.
]
