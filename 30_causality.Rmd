...
This brings our focus to the first level of the causal inference hierarchy: the definition of potential outcomes
Any estimation method that invokes Bayesian tools requires priors for model parameters. 
These priors model the plausibility of parameter values before any data are observed.
Because potential outcomes are functions of causal parameters (such as treatment and control means), and causal parameters require priors, this implies at the very least that causal effects have prior distributions before estimating causal effects from data.
If the Bayesian model contains a unit-level data model, which is the case for most regression approaches, this directly implies that unit-level potential outcomes also have prior distributions.


This is only interesting if I can convince you that we MUST have priors in a causal model, and I'm going to show you that that is true.
This might be no problem if I just give everything a flat prior, but I'm going to show you that that doesn't make sense.
