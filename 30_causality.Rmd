...
One possible retort to this model setup is that a binary outcome should be used instead, such as a logit or probit regression.
These models are typically used to estimate probabilities underlying binary data in other contexts, so perhaps we should entertain its use here.
Although this does run counter to the idea that the regression function is linear in the limit, for the sake of robustness I build a Bayesian logit specification as well. 
...
We accomplish a flat prior for win probabilities at the thredhold using a a standard Logistic prior on the log-odds scale,
\begin{align}
...
Flatness is only a shape, and its relationship to prior information is contingent on other aspects of model configuration.
...
The left panel shows a histogram of $\mathrm{Logistic}\left(0, 1\right)$ simulations, and the right panel shows a histogram of the same values after they are converted to probabilities using the inverse logit link function. 
...
Converting the wide Normal prior to the probability scale, however, shows that higher prior density on logit far from zero translate to probability values very close to $0$ and $1$.
...
\end{align}
because I want the prior to consider a broader distribution slopes on the logit-scale.
The lesson with these priors, as with any prior, is that distributions should be chosen to suit the modeling context.
