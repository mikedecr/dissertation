...
The _conditional independence_ assumption (CIA), also known as conditional "ignorability," states that potential outcomes are independent of treatment status conditional on a unit's membership in a specific stratum of covariates $X_{i} = x$.
\begin{align}
...
Implications derived from identification assumptions are typically posed in terms of expectations about potential outcomes, $\E{Y_{i}\left(A_{i}\right)}$, instead of unit-level potential outcomes $Y_{i}\left(A_{i}\right)$, because unit-level causal heterogeneity is always unidentified [@holland:1986:causal-inf]. 
...
This project will generally discuss _conditional average treatment effects_ (CATEs), which are within-strata expectations of treatment effects averaged over the population of units.
...
In order to separate the major planks of causal inference methods, I center the discussion of causal inference going forward around a three-party hierarchy of causal methodology.
...
2. Identification assumptions: the assumptions required to identify causal estimands using only the observed data. 
   ...
   Because inferences are posed as expectations, they are abstractions that follow from analytical derivations that do not depend on statistical estimation approaches.
...
The prior distribution is commonly described as the researcher's "prior information" (or, controversially, prior "beliefs") about the parameter.  
...
Under the outside view, data follow probability distributions, but parameters are fixed.
Bayesian estimation is nothing but a likelihood model for the data with an added penalty on the parameter estimates, and since the penalty comes from the researcher's subjective beliefs, the penalty could be anything.
Bayesian modeling, under this formulation, can be reduced to a biased estimation procedure that downweights data in favor of arbitrary subjectivity.
...
As I elaborate in Section \@ref(sec:priors), it is a mistake to equate flat prior "flatness" with prior "uninformativeness," and there are many legitimate sources of prior information that have nothing to do with subjective beliefs.
...
Observable estimands are the equivalent quantities in terms of observable data, derived under the identification assumptions, that would be realized if we had an infinite sample size.
...
The transition to a Bayesian probabilistic model begins with an acknowledgment that no estimator for the observable estimand, $\E{Y \mid A = a, X = x}$, is exact. 
...
------------------------->
Let $f(Y_{i} \mid A_{i}, X_{i}, \boldsymbol{\pi}) + \epsilon_{i}$ be a model for $Y_{i}$ that with a systematic predictor depending on treatment $A$, confounders $X$, and parameters $\boldsymbol{\pi}$, and an error term $\epsilon_{i}$.
It follows that the observable estimand is equal to the population expectation of the model.
\begin{align}
  \E{Y \mid A = a, X = x} &= \E{f(Y \mid A = a, X = x, \boldsymbol{\pi}) + \epsilon_{i}}
\end{align}
...
------------------------->
The conditional average treatment effect $\bar{\tau}(X = x)$ is obtained by differencing these model predictions over the treatment.
\begin{align}
\begin{split}
  \bar{\tau}(X = x) 
    &= \E{Y \mid A = 1, X = x} - \E{Y \mid A = 0, X = x} \\
    &= f(Y, A = 1, X = x, \boldsymbol{\pi})
       - f(Y, A = 0, X = x, \boldsymbol{\pi})
\end{split}
(\#eq:cate-f)
\end{align}
...
The expression in (\#eq:prior-cate) represents a prior distribution for $\bar{\tau}(X = x)$, since it is a joint distribution of the CATE and the prior distribution $\p{\boldsymbol{\pi}}$.
Conditioning on the data gives us the posterior distribution for the CATE, $\p{\bar{\tau}(X = x) \mid Y}$, where we would integrate over the posterior distribution of the parameters instead of the prior distribution.
...
\begin{align}
  \p{Y' \mid A = a, X = x} &= \int \p{Y' \mid A = a, X = x, \boldsymbol{\pi}}\p{\boldsymbol{\pi}} \diff{\boldsymbol{\pi}}
  (\#eq:prior-predictive)
\end{align}
...
\begin{align}
  \p{Y' \mid Y, A = a, X = x} &= \int \p{Y' \mid A = a, X = x, \boldsymbol{\pi}}\p{\boldsymbol{\pi} \mid Y} \diff{\boldsymbol{\pi}}
  (\#eq:post-predictive)
\end{align}
...
Let us interpret the Bayesian causal model using the "hierarchy of causal inference" described in Section \@ref(sec:causal-inf).
To review, the hierarchy consisted of three parts:

1. The causal model: definition of potential outcomes, unit-level causal estimands.
2. Identification assumptions: linkage between observable estimands to causal estimands.
3. Estimation: linkage between finite data and observable estimands.
...
## Making Sense of Priors in Causal Inference {#sec:priors}
