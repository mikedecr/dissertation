...
<!------- TO DO ---------
Inside view:
- Conditional probability isn't up for debate (except in a quantum sense).
  It works mathematically. 
  Bayes is "just counting," how many ways can the data come about?
  p(theta, y) intersections of parameters and data, conditioning on $y$ gives you p(theta | y).
  Disputes are about what we choose to describe with probabilities, since p(theta, y) = p(theta | y)p(y) or, gasp, p(theta)p(y | theta)
- Here's the Bayes view: Nothing different about parameters vs. data. 
  Just variables that have probability distributions.
  Likelihoods are priors: quantifying uncertainty with a probability distribution.
- Priors and likelihoods are STRUCTURE for learning from data,
  All learning happens through this structure.
  There is no basis for generalization without priors that relate studies to populations (this is the basis for meta-analysis).
  We have another word for "structure that facilitates learning": a model.
------------------------->

Bayesian analysis is the use of conditional probability to conduct statistical inference.
The controversy surrounding Bayesian methods in political science is not mechanical: probability theory tells us how the probability of an event changes by conditioning on known information.
The controversy arises from different perspectives about which modeling constructs we choose to describe using probabilities.
