...
\begin{align}
  ...
  (\#eq:bayes-prop-log)
\end{align}
MCMC algorithms calculate the right-side of this proportionality at each iteration of the sampler to decide if proposed parameters should be accepted into the sample or rejected. In Stan, this calculation is passed to the _log density accumulator_, a variable containing sum the sum of the log likelihood and log prior density at every sampler iteration [@carpenter-at-al:2016:stan]. 
...
\begin{align}
  \begin{bmatrix}
    \kappa^{*}_{j} \\
    \mathrm{exp}\left(\iota_{j}^{*}\right)
  \end{bmatrix}
  &\sim 
  \mathrm{Normal}\left(\bm{\mu}, \bm{\Sigma}\right)
\end{align}
where $\bm{\mu}$ is a $2$-vector of means and $\bm{\Sigma}$ is a $2 \times 2$ variance-covariance matrix. The hierarchical model for the item parameters allows the model to adaptively estimate the means for the cutpoint and discrimination parameters, rather than "forgetting" the information learned from one item when updating the prior for the next item. This partially pools the item parameters toward their means somewhat.

<!------- TO DO ---------
- I don't actually implement this yet
------------------------->

I give independent Normal hyperpriors to the mean vector and an LKJ prior to the variance-covariance matrix [@LKJ:2009:correlation-matrices],
\begin{align}
  \bm{\mu} &\sim 
    \mathrm{Normal}\left( 
...
\end{align}
where the LKJ scale parameter of $2$ provides weak regularization against off-diagonal covariance terms, whereas $\mathrm{LKJcorr(1)}$ would result in a flat prior over the whole matrix. 
...
ggplot(normal_priors) +
  ...
  facet_wrap(~ sd_label) +
  geom_histogram(boundary = 0, fill = yel) +
