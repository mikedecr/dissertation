...
The choice of quadratic loss implies a utility function over the *squared distance* between an Actor and a choice location. The utility Actor $i$ receives from choosing Right for choice $j$ is $U_{i}\left(\rho_{j}\right) = -\left( \theta_{i} - \rho_{j}\right)^{2} + \nu_{ij}$, while the utility of choosing Left on choice $j$ is $U_{i}\left(\lambda_{j}\right) = -\left( \theta_{i} - \lambda_{j}\right)^{2} + \eta_{ij}$, where $\nu_{ij}$ and $\eta_{ij}$ are the idiosyncratic error terms. I sometimes refer to the quadratic utility loss as the "deterministic" component of the Actor's utility function, while the idiosyncratic error terms are "stochastic" components.^[
...
With these utility functions laid out, we can write Actor $i$'s decision as a comparison of the utilities received by choosing Right or Left. Let $y_{ij} = 1$ signify the outcome that $i$ chooses Right, while $y_{ij} = 0$ signifies that $i$ chooses Left. The model so far implies that $y_{ij} = 1$ if $i$'s utility is greater for Right than for Left.
\begin{align}
  y_{ij} = 1 &\iff U_{i}\left(\rho_{j}\right) > U_{i}\left(\lambda_{j}\right)
  (\#eq:choice-iff)
\end{align}
To visualize this choice, I represent the deterministic components of Equation&nbsp;\@ref(eq:choice-iff) in Figure&nbsp;\@ref(fig:plot-utility), with idiosyncratic utility terms omitted. The parabola represents $i$'s fixed utility loss for any choice along the ideological continuum, owed to her distance from that choice. The vertex of the parabola is at the Actor's location, indicating that she would maximize her (spatial) utility if she could choose a policy located exactly at her ideal point. Dashed lines below the Left and Right alternatives represent the utility loss owed to the Actor's distance from those specific choices. In the current example, the Actor is closer to Right than to Left, so she receives greater utility (or, less utility *loss*) by choosing Right instead of Left.
...
\begin{align}
  ...
  \mathrm{Pr}\left(y_{ij} = 1\right) &= 
    \mathrm{Pr}\left(U_{i}\left(\rho_{j}\right) > U_{i}\left(\lambda_{j}\right) \right) \\
  &=
    \mathrm{Pr}\left( 
      -\left( \theta_{i} - \rho_{j}\right)^{2} + \nu_{ij} > 
      -\left( \theta_{i} - \lambda_{j}\right)^{2} + \eta_{ij}
    \right) \\
  &= \mathrm{Pr}\left(
      \left( \theta_{i} - \lambda_{j}\right)^{2} -
            \left( \theta_{i} - \rho_{j}\right)^{2} 
        > \eta_{ij} - \nu_{ij}
      \right)
\end{split}
(\#eq:choice-prob)
\end{align}  
The intuition for \@ref(eq:choice-prob) is that the Actor will choose the policy alternative that is nearest to her, *unless* idiosyncratic (non-policy) factors overcome her ideological considerations. Supposing that the Actor is closer to Right than to Left, $\left( \theta_{i} - \lambda_{j}\right)^{2}$ will be larger than $\left( \theta_{i} - \rho_{j}\right)^{2}$, meaning that the left-hand side of the inequality (the deterministic component) will be some positive value. The only way for $i$ to choose Left would be if the idiosyncratic utility of Left over Right exceeded the deterministic utility of Right over Left. 
  <!-- to do: assumes? -->
...
Equation&nbsp;\@ref(eq:choice-prob) can be rearranged to reveal an appealing appealing functional form for $i$'s choice probability. Expanding the polynomial terms on the left side of the inequality...
\begin{align}
\begin{split}
  \mathrm{Pr}\left(y_{ij} = 1\right) &= 
    \mathrm{Pr}\left(
         ...
         \right) \\
  &= \mathrm{Pr}\left(
      \theta_{i}^{2} - 2\theta_{i}\lambda_{j} + \lambda_{j}^{2} 
      - \theta_{i}^{2} + 2\theta_{i}\rho_{j} - \rho_{j}^{2}
      > \eta_{ij} - \nu_{ij} 
    \right) \\
  &= \mathrm{Pr}\left(
      2\theta_{i}\rho_{j} - 2\theta_{i}\lambda_{j} + 
      \lambda_{j}^{2} - \rho_{j}^{2}
      > \eta_{ij} - \nu_{ij} 
    \right) \\
  &= \mathrm{Pr}\left(
      2(\rho_{j} - \lambda_{j})\theta_{i} +
      \lambda_{j}^{2} - \rho_{j}^{2}
      > \eta_{ij} - \nu_{ij} 
    \right) \\
  &= \mathrm{Pr}\left(
      2(\rho_{j} - \lambda_{j})\theta_{i} +
      \left(\rho_{j} - \lambda_{j}\right)\left(\rho_{j} + \lambda_{j}\right)
      > \eta_{ij} - \nu_{ij} 
    \right) \\
  &= \mathrm{Pr}\left(
      2\left(\rho_{j} - \lambda_{j}\right)
      \left(\theta_{i} - \frac{\rho_{j} + \lambda_{j}}{2}\right)
      > \eta_{ij} - \nu_{ij} 
    \right) \\
  &= \mathrm{Pr}\left(
...
(\#eq:choice-2p)
\end{align}
The final line of Equation&nbsp;\@ref(eq:choice-2p) simplifies the model by introducing the "discrimination parameter" $\beta_{j}$, the "midpoint" or "cutpoint" parameter $\kappa_{j}$, and $\epsilon_{ij}$.^[
  Referring to these parameters as "discrimination" and "difficulty" parameters is inherited from item-response theory (IRT), an area of psychometrics that is similarly interested in inferring latent traits from observed response data. In the next section I show the relationship between the difficulty parameter and the cutpoint parameter.
]
Parameterizing the model in this way expresses the utility comparison in a simpler, linear form. Following @clinton-jackman-rivers:2004:ideal, we obtain these new parameters by making a parametric assumption for $\epsilon_{ij}$. Assume that $\epsilon_{ij}$ were a draw from a Logistic distribution with mean of $0$ and scale of $s_{ij}$. This would imply that $\mathrm{E}\left[\eta_{ij}\right] = \mathrm{E}\left[\nu_{ij}\right]$ and that $\mathrm{Var}\left[\eta_{ij} - \nu_{ij}\right] = \dfrac{s_{ij}^{2}\pi^{2}}{3}$. With scale $s_{ij}$ in hand, let $\beta_{j} = \dfrac{1}{s_{ij}}2(\rho_{j} - \lambda_{j})$, let $\kappa_{j} = \dfrac{1}{s_{ij}}\left(\dfrac{\lambda_{j} + \rho_{j}}{2}\right)$, and let $\epsilon_{ij} = \dfrac{1}{s_{ij}}\left(\eta_{ij} - \nu_{ij}\right)$. I discuss the intuitions surrounding these new parameters $\kappa_{j}$ and $\beta_{j}$ in the next section.

The parametric assumption for $\epsilon_{ij}$ reveals how the utility model reduces to a logistic regression model that is common in political science,
\begin{align}
\begin{split}
  \mathrm{Pr}\left(y_{ij} = 1\right) &= 
  \mathrm{Pr}\left(
  ...
  \right) \\
  &= 
  \mathrm{logit}^{-1}\left( \beta_{j}\left[\theta_{i} - \kappa_{j}\right] \right),
\end{split}
(\#eq:logit-irt)
\end{align}
where $\mathrm{logit}^{-1}(\cdot)$ is the cumulative Logistic distribution function. This deviates slightly from @clinton-jackman-rivers:2004:ideal who assume that $\epsilon_{ij}$ is a draw from a standard Normal distribution rather than a standard Logistic distribution, a decision that instead implies a probit model,
\begin{align}
  \mathrm{Pr}\left(y_{ij} = 1\right) &= 
  \Phi\left( \beta_{j}\theta_{i} + \kappa_{j} \right).
  (\#eq:probit-irt)
\end{align}
where $\Phi\left(\cdot\right)$ is the cumulative Normal distribution function.^[
  A technical point of difference between the probit and logit model is the way parameters are scaled to yield the final line of Equation&nbsp;\@ref(eq:choice-2p). If it is assumed that $\epsilon_{ij}$ is a standard Normal draw, the expression for $s_{ij}$ is simpler ($s^2_{ij} = \mathrm{Var}\left[\eta_{ij} - \nu_{ij}\right]$), although the probit model overall is more computationally burdensome. 
]
<!-- 
\begin{align}
  \mathrm{Pr}\left(y_{ij} = 1\right) &= 
  \mathrm{logit}^{-1}\left( \beta_{j}\theta_{i} + \kappa_{j} \right).
  (\#eq:logit-irt)
\end{align}
 -->
I opt for the logit model for computational reasons described in Section&nbsp;\@ref(#sec:stan-setup).


<!-- Normal: $s_{j} = \sqrt{\mathrm{Var}\left[\eta_{ij} - \nu_{ij}\right]}$

Logistic: $s_{j} = \frac{\sqrt{3 \mathrm{Var}\left[\eta_{ij} - \nu_{ij}\right]}}{\pi}$ -->
