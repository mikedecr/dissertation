...
\begin{align}
  \kappa_{j} &= \tilde{\kappa}_{j} - \frac{1}{J}\sum\limits_{j = 1}^{J} \tilde{\kappa}_{j},
  (\#eq:scale-midpoint)
\end{align}
which is performed in every iteration of the sampler. 
This restriction on the sum of the cutpoint parameters also implies a restriction on the mean of the cutpoints, since $\frac{1}{J}0 = 0$.
...
I implement this by restricting the log discrimination parameter to have a sum of $0$, which achieves an equivalent transformation.^[
  A quick demonstration using three unknown values $a$, $b$, and $c$. If $a \times b \times c = 1$, then $\log(a) + \log(b) + \log(c) = \log(1)$, where $\log(1) = 0$.
]
Letting $\tilde{\iota}_{j}$ be the unrestricted difficulty parameter, we obtain the restricted $\iota_{j}$ as follows.
\begin{align}
...
Thirdly, model parameterization is important for effective Bayesian computation, and although reparameterizations do not affect the likelihood of data given the parameters, parameterization naturally affects the choice of priors.
...
Important elements of a Bayesian modeling workflow, including simulations using fake data and the prior distributions for model parameters [@betancourt:2018:workflow-blog;@gabry-et-al:2019:visualization].
...
```{r plot-valid-probit, include = TRUE, fig.width = 5, fig.height = 4, out.width = "70%", fig.cap = "The region of the probit model's linear index that maps to response probabilities between 0 and 99 percent."}
ggplot(prior_probit) +
