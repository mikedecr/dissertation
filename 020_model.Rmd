...
\begin{align}
  \log p\left(s^{*}_{gj} \mid \pi_{gj} \right) \propto s^{*}_{gj} \log \pi_{gj} + \left(n_{gj}^{*} - s^{*}_{gj}\right) \log \left(1 -\pi_{gj}\right)
  (\#eq:weighted-ll-raw)
\end{align}
...
\end{align}
where $\bm{\mu}$ is a vector of means and $\bm{\Sigma}$ is a variance-covariance matrix. I give independent Normal hyperpriors to the means and an LKJ prior to the variance-covariance matrix [@LKJ:2009:correlation-matrices],
\begin{align}
  \bm{\mu} &\sim 
    \mathrm{Normal}\left( 
