...
Equation&nbsp;\@ref(eq:choice-2p) simplifies the model by introducing the "discrimination parameter" $\beta_{j}$, the "difficulty parameter" $\alpha_{j}$, and $\epsilon_{ij}$.^[
  Referring to these parameters as "discrimination" and "difficulty" parameters is inherited from item-response theory (IRT), an area of psychometrics that is similarly interested in inferring latent traits from observed response data.
]
Following @clinton-jackman-rivers:2004:ideal, it is assumed that utility errors $\eta_{ij}$ and $\nu_{ij}$ are distributed such that $\mathrm{E}\left[\eta_{ij}\right] = \mathrm{E}\left[\nu_{ij}\right]$ and that $\mathrm{Var}\left[\eta_{ij} - \nu_{ij}\right] = \sigma^{2}_{j}$. Let $\beta_{j} = \dfrac{1}{\sigma_{ij}}2(\rho_{j} - \lambda_{j})$, let $\alpha_{j} = \dfrac{1}{\sigma_{ij}}\left(\lambda_{j}^{2} - \rho_{j}^{2}\right)$, and let $\epsilon_{ij} = \dfrac{1}{\sigma_{ij}}\left(\eta_{ij} - \nu_{ij}\right)$. Parameterizing the model in this way expresses the utility comparison in a simpler, linear form. The final line reveals how the utility model reduces to binary choice models that are common in political science. Assuming as @clinton-jackman-rivers:2004:ideal do that $\epsilon_{ij}$ is a draw from a standard Normal distribution, the final line in Equation&nbsp;\@ref(eq:choice-2p) implies a probit model,
\begin{align}
  \mathrm{Pr}\left(y_{ij} = 1\right) &= 
  \Phi\left( \beta_{j}\theta_{i} + \alpha_{j} \right).
  (\#eq:probit-irt)
\end{align}
where $\Phi\left(\cdot\right)$ is the cumulative Normal distribution function. If we instead assume that $\epsilon_{ij}$ is a draw from a standard Logistic distribution,^[
  A technical point of difference between the probit and logit model is the way parameters are scaled to yield the final line of Equation&nbsp;\@ref(eq:choice-2p). This is because the scale parameter in a logistic distribution is not its standard deviation but a function of the standard deviation. If $\sigma_{j}^2 = \mathrm{Var}\left[\eta_{ij} - \nu_{ij}\right]$, then the logit model is created by scaling by $s_{j} = \frac{\sqrt{3}}{\pi}\sigma_{j}$ instead of by $\sigma_{j}$.
]
we obtain a logit model.
\begin{align}
  \mathrm{Pr}\left(y_{ij} = 1\right) &= 
  \mathrm{logit}^{-1}\left( \beta_{j}\theta_{i} + \alpha_{j} \right).
  (\#eq:logit-irt)
\end{align}
For computational purposes described in Section&nbsp;\@ref(#sec:stan-setup), I use a logit model. I discuss the intuition for these new parameters $\alpha_{j}$ and $\beta_{j}$ in the next section.
...
Past ideal point modeling in political science has noted the similarity between the logit and probit models and models developed under item response theory (IRT) in psychometrics [for example, @londregan:1999:ideal-pts]. The mission of IRT models in psychometrics is similar to ideal point models in political science: measuring latent features in the data given individuals' response patterns to various stimuli. The canonical example is in education testing, where tests are used to measure a student's unobserved academic "ability" level. This section describes the intuition of IRT models in the educational testing context, important mathematical features of the model's functional form assumptions, and extends the IRT model's intuition into the context of this project: survey responses about policy preferences.
