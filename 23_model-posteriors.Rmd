...
I initialize the algorithm with an `adalt_delta` parameter of `r adapt_delta` and a maximum proposal tree-depth of `r max_treedepth`.
...
Just `r sum(rstan::get_divergent_iterations(mcmc))` out of `r comma(total_samples)` iterations (`r mean(rstan::get_divergent_iterations(mcmc)) %>% percent(accuracy = .001)`) encountered a divergent transition, which indicates no systematic issues with model parameterization,
...
```{r plot-theta-rank, include = TRUE, fig.scap = "District-party ideal points and rank-ordering.", fig.cap = "District-party ideal points and rank-ordering. Points are posterior means. Error bars are 50 and 90 percent compatibility intervals.", out.width = "100%", fig.height = 5, fig.width = 7}
```
...
ggplot(joined_ideal) +
  ...
  labs(
    ...
    x = "CCES self-placement, district-party average (2012",
...
```{r plot-hist-means, include = TRUE, fig.cap = "Histogram of ideal point means in both parties.", fig.scap = "Histogram of ideal point means in both parties."}
```
...
```{r plot-sds, include = TRUE, fig.scap = "Geographic heterogeneity in ideal points.", fig.cap = "Geographic heterogeneity in ideal points. Left panel: standard deviation of Republican and Democratic district-party ideologies. Right panel: difference in standard deviations (Democrats minus Republicans). Distributions reflect MCMC draws.", fig.width = 9, fig.height = 4, out.width = "100%"}
```
...
It is natural to wonder if Republican and Democratic ideal points covary:
do the districts containing more conservative Republicans also contain more conservative Republicans? 
Ideal points may be positively correlated if broad demographic and cultural features of a district (e.g. race, urbanity/rurality) are related to ideology in both parties in similar ways.
...
The overwhelming takeaway is that a large majority of the variation in Republican ideal points is unrelated to Democrat ideal points.

```{r plot-cors, include = TRUE, fig.height = 5, fig.width = 10}
```
...
If Republican and Democratic ideal points are largely unrelated, it may be because aggregate data about states and districts are related to partisan ideology in diverging ways. 
One final piece of descriptive analysis is to explore the relationship between district-party ideal points and the hierarchical covariates used to improve the precision of the ideal point estimates.

```{r plot-party-covs, include = TRUE, out.width = "100%", fig.height = 7, fig.width = 12}
```

- white: 
    - coef: makes GOP more conservative and Dems LEAn more prog
    - marginally: large increase among GOP, but Dems suggest other variables at work
- unemployed:
    - coef: more conservative dems, more liberal republicans
    - marginal: same
- Foreign both:
    - coef: more liberal Republicans (cosmopolitan Republicans?)
- college:
    - coef: more liberal in both parties!
    - marginal: this shows up for Democrats only! But the effect is HUGE. Among GOP it appears that other things are at work
- Income: 
    - coef: both more conservative
    - marginal: FLIPPED for Dems. Other things about wealthy Dem districts make those districts more liberal (education maybe)
- Inequality:
    - coef: more conservative GOP
    - marginal: flipped for GOP
