...
mediator_dag <- base_dag +
  geom_dag_node(data = filter(g_dag, name %in% c("U2", "U1") == FALSE)) +
  geom_dag_edges(
...
exposure_dag <- base_dag +
  ...
  geom_dag_text(
    ...
    aes(label = ifelse(name == "C", "C*", name))
  ) +
  labs(title = "\nStage 2", subtitle = "Exposure model with\ndemediated outcome")
  NULL

confounding_dag <- base_dag +
  geom_dag_node(aes(color = name %in% c("U2", "U1"))) +
  geom_dag_edges() +
  ...
  labs(title = "Violations of Sequential\nUnconfoundedness", 
       subtitle = "In mediator model (U1)\nand exposure model (U2)")
```
...
Figure&nbsp;\@ref(fig:plot-g-dag) outlines the modeling assumptions for sequential-$g$ estimation.

- stage 1
- stage 2, deletes the paths

```{r plot-g-dag, include = TRUE, fig.width = 11, fig.height = 5, out.width = "100%", fig.cap = "Directed acyclic graphs that describe the modeling problem and sequential-$g$ estimation. The stage 1 mediator model estimates the effect of the past presidential vote ($V$) on candidate positioning ($C$). The stage 2 exposure model subtracts the vote effect from candidate positions and estimates the effect of partisan group ideology ($T$) on *demediated* candidate positions ($C^{*}$). The final panel shows where unadjusted confounders violate the identification assumptions in stage 1 ($U1$) and in stage 2 ($U2$)."}
```
...
We care about the total effect and the (controlled) direct effect.
...
- Draw a sample of $n_{\bar{\theta}}$ from posterior $p\left(\bar{\theta}\right)$
- For each $\bar{\theta}^{*}$: 
    - estimate mediator model
    - extract the mediator's effect on the dependent variable. To carry uncertainty forward, do this by sampling $1$ draw from the coefficient's distribution (multinorm for all coefficients). (This results in a distribution of mediator effects that reflect posterior uncertainty over $\bar{\theta}$ and in the mediator effect)
    - Calculate the blipdown function and use it to demediate $y$. (This creates a distribution of demediated $y$  values)
    - Use demediated $y$ to estimate direct effect of $\bar{\theta}$. This results in a distribution of ACDEs, reflecting uncertainty in $\bar{\theta}$, $\delta$, and the ACDE.
