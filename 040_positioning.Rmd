...
```{r read-dime}
dime <- read_rds(here("data", "clean", "dime-with-means.rds"))
```


```{r simple-regs}
simple_regs <- 
  read_rds(here("data", "model-output", "04-positioning", "simple-regs.rds"))
```
...
```{r plot-scattering, include = FALSE, fig.width = 5, fig.height = 4, out.width= "70%", fig.cap = "Presidential Vote Shares and Ideal Point Measures, 2012--2016"}
```



```{r plot-scatter-by-incumbency}
ggplot(dime) +
  aes(
    x = district_pres_vs, y = recipient_cfscore_dyn, 
    color = as.factor(party)
...
```{r plot-mediator, include = TRUE, fig.width = 7, fig.height = 3, out.width = "100%", fig.cap = "Mediator findings"}
```
...
```{r plot-direct, include = TRUE, fig.width = 7, fig.height = 3, out.width = "100%", fig.cap = "ACDE findings"}
```
...
```{r plot-indirect, include = TRUE, fig.width = 7, fig.height = 3, out.width = "100%", fig.cap = "ATE-ACDE findings"}
```
