# Primary Elections and Ideological Choice 
\label{arg-chapter}
...
```{r, include = TRUE}
tibble(x = seq(-3, 3, .005)) %>%
  mutate(
    dem_moderate = dnorm(x, mean = -0.5, sd = 0.5),
    rep_moderate = dnorm(x, mean = 0.5, sd = 0.5),
    dem_extreme = dnorm(x, mean = -1, sd = 0.5),
    rep_extreme = dnorm(x, mean = 1, sd = 0.5),
  ) %>%
  gather(key = group, value = density, -x) %>%
  mutate(
    party = ifelse(str_detect(group, "dem"), "Democrats", "Republicans"), 
    extremity = ifelse(str_detect(group, "mod"), "Moderate Parisans", "Polarized Partisans")) %>%
  ggplot() + 
    aes(x = x, y = density, fill = party, color = party) +
    geom_ribbon(
      aes(ymin = 0, ymax = density), 
      alpha = 0.5
    ) +
    geom_line() +
    facet_wrap(~ extremity, scales = "free") +
    scale_fill_manual(values = partisans_colors) +
    scale_color_manual(values = partisans_colors)
```
