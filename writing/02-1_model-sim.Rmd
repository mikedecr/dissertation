...
# perhaps unload these things after?
load(here("data", "mcmc", "dgirt", "test", "input", "sim-params.Rdata"))
```
...
load(here("data", "mcmc", "dgirt", "test", "input", "mcmc-params.Rdata"))

# but don't trust local n of chains; obtain from stanfit obj
n_chains <- stanfit %>% attributes() %$% stan_args %>% length()
...
```{r tidy-stanfit}
tidy_stanfit <- tidy(stanfit, conf.int = TRUE)
```
...
```{r theta-scatters, include = TRUE, fig.height = 4, fig.width = 5, out.width = "70%", fig.cap = 'Testing the ideal point model on simulated data: estimated vs. "true" ideal points for district-party groups'}
...
ggplot(thetas) +
  ...
  scale_color_manual(values = c(yel, purp)) +
...
icc_draws %>%
  ggplot() +
  aes(x = theta, y = pp) +
  geom_line(aes(group = .draw), color = yel, size = 0.1) +
  facet_wrap(
    ~ item,
  ...
  geom_line(
    data = ij_level, color = "black", size = 0.25,
    aes(x = theta_i, y = pi_ij)
  ) +
