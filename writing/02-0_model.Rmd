...
\begin{align}\label{eq:choice-iff}
...
It is important to remember that Equation&nbsp;\@ref(eq:choice-iff) shows only the deterministic component of choice task $j$; random error components $\nu_{ij}$ and $\eta_{ij}$ are suppressed. With idiosyncratic utility error incorporated, Equation&nbsp;\@ref(eq:choice-iff) implies that there is still a probability that Actor $i$ chooses Left, but the probability depends on the instantiated values of the error terms. We can restate the insight from Equation&nbsp;\@ref(eq:choice-iff) probabilistically:
\begin{align}
  \begin{split}\label{eq:choice-prob}
...
The structure of Equation&nbsp;\@ref(eq:choice-prob) is the basis for a statistical model of the choice between Right and Left. 
\begin{align}
\begin{split}\label{eq:choice-manip}
  \mathrm{Pr}\left(y_{ij} = 1\right) &= 
    \mathrm{Pr}\left( 
      -\left( \theta_{i} - \psi_{j}\right)^{2} + \nu_{ij} > 
      -\left( \theta_{i} - \chi_{j}\right)^{2} + \eta_{ij}
...
\end{split}
\end{align}  
The final line of \@ref(eq:choice-manip) reparameterizes the utility comparison into a simpler and familiar linear form by defining a "discrimination" parameter $\beta_{j} = \dfrac{1}{\sigma_{ij}}2(\psi_{j} - \chi_{j})$, a "difficulty" parameter $\alpha_{j} = \dfrac{1}{\sigma_{ij}}\left(\chi_{j}^{2} - \psi_{j}^{2}\right)$, and difference in errors $\epsilon_{ij} = \eta_{ij} - \nu_{ij}$.^[
...
The discrimination parameter $\beta_{j}$ grows in magnitude as the difference between the Right and Left alternatives grows larger. Because it serves as a "slope" in the final line of Equation&nbsp;\@ref(eq:choice-manip), one intuition is that $i$'s choice places greater *weight* on ideological considerations if the policy alternatives are increasingly distant in ideological space. The difficulty parameter, meanwhile, increases as either $\chi_{j}$ or $\psi_{j}$ increase (that is, move to the right). An intuition for this parameter is that as the choices become more conservative on average,   


 for the is that $i$'s choice places greater weight 


If error components $\nu_{ij}$ and $\eta_{ij}$ are jointly normal, mean zero, and uncorrelated, it follows that $\epsilon_{ij}$ is also normal, so Equation&nbsp;\@ref(eq:choice-manip) yields a probit model,
\begin{align}
  \mathrm{Pr}\left(y_{ij} = 1\right) &= 
    \Phi\left( \beta_{j}\theta_{i} + \alpha_{j} \right),
\end{align}
where $\Phi\left(\cdot\right)$ represents the standard Normal cumulative distribution function. Equation&nbsp;\@ref(eq:choice-manip) results in a logistic model if $\epsilon_{ij}$ is a draw from standard logistic distribution, which is derived from an assumption that error components $\nu_{ij}$ and $\eta_{ij}$ are distributed type-1 extreme value [@clinton-jackman-rivers:2004:ideal].
\todo{logit}
<!-- two things. -->
